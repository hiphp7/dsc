...
class Image
{
    ...
    public function add_watermark($filename, $target_file = '', $watermark = '', $watermark_place = '', $watermark_alpha = 0.65)
    {
        ...
        $watermark = storage_public($watermark);
    ...
    public function validate_image($path)
    {
        ...
        if (strpos($path, 'http') !== false) {
            if (strpos($path, 'storage') !== false) {
                $watermark_file = str_replace(asset('/') . 'storage/', '', $path);
            } else {
                $watermark_file = str_replace(asset('/'), '', $path);
            }
        } else {
            $watermark_file = $path;
        }
        ...
        /* 文件是否存在 */
        if ($GLOBALS['_CFG']['open_oss'] == 0 && !file_exists(storage_public($watermark_file))) {
