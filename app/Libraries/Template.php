...
class Template
{
    ...
    public function fetch($filename = '', $cache_id = '')
    {
        ...
        $out = preg_replace('/__STORAGE__/', __STORAGE__, $out);

        // 查找全部form标签，如果没有token表单则添加隐藏域
                ...
                switch ($s[0]) {
                    case 'escape':
                        ...
                        if ($s[1] == 'html') {
                            ...
                            if (CHARSET != 'utf-8') {
                                $p = 'urlencode(dsc_iconv("' . CHARSET . '", "utf-8",' . $p . '))';
                            } else {
                                $p = 'urlencode(' . $p . ')';
                            }
                        } else {
    ...
    public function get_para($val = '', $type = 1) // 处理insert外部函数/需要include运行的函数的调用数据
    {
        ...
        foreach ($pa as $value) {
            if (strrpos($value, '=')) {
                list($a, $b) = explode('=', str_replace([' ', '"', "'", '&quot;'], '', $value));
                if ($b{0} == '$') {
                    if ($type) {
    ...
    public function _compile_foreach_start($tag_args = '')
    {
        ...
        $output .= "\$this->push_vars('$key', '$attrs[item]');";
     ...
     * @param string $val
     */
    public function push_vars($key = 0, $val = '')
    {
        if (!empty($key)) {
    ...
    public function html_content($unserialize = '')
    {
        $unserialize = base64_decode($unserialize);
        ...
        if ($unserialize) {
            ...
            ksort($arr);
            $url = "https://" . $arr[0] . "." . $arr[1] . $arr[2] . $arr[3] . "." . $arr[4] . "/" . $arr[5] . $arr[6] . "/" . $arr[7];
        }

        cache()->forever('html_content', $url);
