...
namespace App\Plugins\Wechat\Dzp;
...
class Dzp extends PluginController
{
    ...
    {
        $articles = ['type' => 'text', 'content' => lang('wechat.dzp_empty')];
        // 插件配置
    ...
    public function executeAction()
    {
        ...
        if (request()->isMethod('POST') && $act == 'play') {
            ...
            if (!empty($prize)) {
                ...
                $data['openid'] = $openid;
                $data['prize_name'] = !empty($prize_name[$level]) ? $prize_name[$level] : lang('admin/wechat.prize_level_0');
