...
<div class="grid">
    ...
    <div class="block">
        ...
        @foreach($data['prize'] as $v)
            ...
            <p>{{ $v['prize_level'] }}:{{ $v['prize_name'] }}({{ $lang['zjd_prize_number'] }}：{{ $v['prize_count'] }})</p>

        @endforeach

    </div>
    <div class="block">
        <div class="title">{{ $lang['my_prize_log'] }}</div>
    ...
    <div class="block">
        <div class="title">{{ $lang['activity_rules'] }}</div>
...
<script>
    $(function () {
        ...
        function wxch_result() {
            var url = "{{ route('wechat/plugin_action', ['name' => 'zjd', 'wechat_ru_id' => $wechat_ru_id]) }}";
