		...
		<div class="flexilist">
			<div class="main-info">
				...
				<div class="switch_info">
				    ...
				    <div class="item">
				        <div class="label-t">{{ $lang['bonus_status'] }}：</div>
				            <div class="label_value">
				            	<div class="checkbox_items">
		                            <div class="checkbox_item">
					                    <input type="radio" name="cfg_value[bonus_status]" class="ui-radio event_zhuangtai" id="value_120_0" value="1" checked="true"
@if(isset($config['config']['bonus_status']) && $config['config']['bonus_status'])
checked
@endif
>
					                    <label for="value_120_0" class="ui-radio-label
@if(isset($config['config']['bonus_status']) && $config['config']['bonus_status'])
active
@endif
">{{ $lang['wechat_open'] }}</label>
					                ...
					                <div class="checkbox_item">
					                    <input type="radio" name="cfg_value[bonus_status]" class="ui-radio event_zhuangtai" id="value_120_1" value="0"
@if(isset($config['config']['bonus_status']) && empty($config['config']['bonus_status']))
checked
@endif
>
					                    <label for="value_120_1" class="ui-radio-label
@if(isset($config['config']['bonus_status']) && empty($config['config']['bonus_status']))
active
@endif
">{{ $lang['wechat_close'] }}</label>
				    ...
				    <div class="item">
				        <div class="label-t">{{ $lang['select_bonus_type'] }}：</div>
			            <div class="label_value">
			            	<div class="select_w320 fl" style="margin-right:10px;">
				                <select name="cfg_value[bonus]" class="form-control">
				                    <option value="0">{{ $lang['please_select'] }}...</option>

@foreach($config['bonus'] as $v)

				                    <option value="{{ $v['type_id'] }}"
@if(isset($config['config']['bonus']) && ($config['config']['bonus'] == $v['type_id']))
selected
@endif
>{{ $v['type_name'] }} => {{ $lang['money'] }}：{{ $v['type_money'] }}</option>

@endforeach

				                </select>
			                </div>
			                <div class="notic">{!! $lang['select_bonus_type_notice'] !!}</div>
			           </div>
				    </div>
				    <div class="item">
