...
<div class="wrapper-right of">
	...
	<div class="wrapper-list mt20" >
		<div class="common-head">
            <div class="fl mb20">
            	<a href="{{ route('seller/wechat/data_list', array('type' => $config['keywords'], 'function' => 'shake', 'handler' => 'edit', 'id' => $config['market_id'])) }}" class="sc-btn sc-blue-btn" ><div class="fbutton"><div class="add " title="添加广告"><span><i class="fa fa-plus"></i>添加广告</span></div></div></a>
            </div>
        </div>
...
@foreach($list as $advertice)
                    ...
                    <tr class="text-center adv-list">
                        ...
                        <td class="handle">
                        <div class="tDiv a2">
                            <a class="btn_edit fancybox fancybox.iframe" href="{{ route('seller/wechat/data_list', array('type' => $config['keywords'], 'function' => 'shake', 'handler' => 'edit', 'id' => $config['market_id'], 'advertice_id' => $advertice['id'])) }}"  title="编辑" ><i class="fa fa-edit"></i>{{ $lang['wechat_editor'] }}</a>
                            <a class="btn_trash shake_delete" href="javascript:;" data-href="{!!  route('seller/wechat/market_action', array('type' => $config['keywords'], 'handler' => 'shake_delete', 'market_id' => $config['market_id'], 'advertice_id' => $advertice['id']))  !!}"  title="删除" ><i class="fa fa-trash-o"></i>{{ $lang['drop'] }}</a>
                        </div>
                        </td>
                    </tr>
