...
@foreach($list as $val)
            ...
            <tr class="text-center wall-list">
                ...
                <td class="handle">
                <div class="tDiv">
                    <a class="btn_edit" href="{{ route('seller/wechat/market_edit', array('type' => $config['keywords'], 'id' => $val['id'], 'wechat_ru_id' => $wechat_ru_id)) }}"><i class="fa fa-edit"></i>编辑</a>
                    <a class="btn_see" href="{{ route('seller/wechat/data_list', array('type' => $config['keywords'], 'function' => 'messages', 'id' => $val['id'], 'status' => 0, 'wechat_ru_id' => $wechat_ru_id)) }}"><i class="fa fa-eye"></i>消息</a>
                    {{--<a class="btn_see" href="{{ route('wechat/market_show', array('type' => $config['keywords'], 'function' => 'wall_msg', 'wall_id' => $val['id'], 'wechat_ru_id' => $wechat_ru_id)) }}" target="_blank"><i class="fa fa-desktop"></i>大屏幕</a>--}}
                    <a class="btn_see fancybox-img fancybox-img.iframe getqr" href="{{ route('seller/wechat/market_qrcode', array('type' => $config['keywords'], 'id' => $val['id'], 'wechat_ru_id' => $wechat_ru_id)) }}"><i class="fa fa-qrcode"></i>上墙二维码</a>
                    <a class="btn_trash delete_confirm" data-href="{!!  route('seller/wechat/market_del', array('type' => $config['keywords'], 'id' => $val['id'], 'wechat_ru_id' => $wechat_ru_id))  !!}" href="javascript:;"><i class="fa fa-trash-o"></i>删除</a>
                </div>
                </td>
            </tr>
