...
<div class="wrapper">
	<div class="title"><a href="{{ route('admin/wechat/market_list', array('type' => $config['keywords'])) }}" class="s-back">返回</a>{{ $lang['wechat_menu'] }} - {{ $config['name'] }}记录</div>
...
@if($config['status'])
 class="curr"
@endif
><a href="{{ route('admin/wechat/data_list', array('type' => $config['keywords'], 'function' => 'messages', 'id' => $config['market_id'], 'status' => 'all')) }}">全部消息</a></li>
                <li
@if(empty($config['status']))
 class="curr"
@endif
><a href="{{ route('admin/wechat/data_list', array('type' => $config['keywords'], 'function' => 'messages', 'id' => $config['market_id'], 'status'=> '')) }}">未审核消息</a></li>
		...
		<form action="{{ route('admin/wechat/market_action', array('type' => $config['keywords'], 'function' => 'messages', 'handler' => 'batch_checking','market_id' => $config['market_id'], 'status' => $config['status'])) }}" method="post" class="form-inline" role="form">
...
@foreach($list as $val)
                            ...
                            <tr class="text-center wall-list">
                                <td class="sign"><input type="checkbox" name="id[]" value="{{ $val['msg_id'] }}" class="checks"></td>
                                <td>{{ $val['nickname'] }}</td>
...
@if($config['status'] == 'all' && $val['status'] == '已审核')
                                    ...
                                    <a class="button btn-warning move" data-href="{!!  route('admin/wechat/market_action', array('type' => $config['keywords'], 'function' => 'messages', 'handler' => 'move','market_id' => $config['market_id'], 'msg_id' => $val['msg_id'], 'user_id' => $val['user_id'], 'status' => 'all'))  !!}" href="javascript:;">移除</a>

@endif

                                    <a class="button btn-danger bg-red data_delete" data-href="{!!  route('admin/wechat/market_action', array('type' => $config['keywords'], 'handler' => 'data_delete', 'market_id' => $config['market_id'], 'msg_id' => $val['msg_id'], 'user_id' => $val['user_id']))  !!}" href="javascript:;">删除</a>
                                </td>
                            </tr>
