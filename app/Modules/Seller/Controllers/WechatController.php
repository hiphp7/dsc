...
class WechatController extends BaseController
{
    ...
    protected function initialize()
    {
        ...
        if (empty($wechat)) {
            return $this->message(lang('seller/wechat.config_wxt'), null, 2, true);
        }
        ...
        // 初始化新增商家公众号
        $seller_wechat = $this->wechatManageService->getWechatConfigByRuId($this->ru_id);
        if (!empty($seller_wechat)) {
            $without = [
                'index',
                'append',
                'modify',
            ...
            if (!in_array(strtolower($currentMethod), $without)) {
                if (isset($seller_wechat['status']) && $seller_wechat['status'] == 0) {
                    return $this->message(lang('admin/wechat.open_wechat'), route('seller/wechat/modify'), 2, true);
                }
            }
            // 微信对象
            $this->weObj = $this->wechatManageService->wechatInstance($seller_wechat);
            $this->wechat_type = $seller_wechat['type'];
            $this->wechat_id = $seller_wechat['id'];
        } else {
            // 新增商家微信
            $data = [
                'type' => 2,
            ...
            if ($currentMethod != 'index' && $currentMethod != 'modify') {
                return redirect()->route('seller/wechat/modify');
            }
        }
    ...
    public function modify()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            if (!$form->isEmpty($data['name'], 1)) {
                return $this->message(lang('admin/wechat.must_name'), null, 2, true);
            }
            if (!$form->isEmpty($data['orgid'], 1)) {
                return $this->message(lang('admin/wechat.must_id'), null, 2, true);
            }
            if (!$form->isEmpty($data['token'], 1)) {
                return $this->message(lang('admin/wechat.must_token'), null, 2, true);
            }
            // 更新数据
            $this->wechatManageService->updateWechat($this->wechat_id, $data);

            return $this->message(lang('admin/wechat.wechat_editor') . lang('admin/common.success'), route('seller/wechat/modify'), 2, true);
        }
    ...
    public function menu_edit()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            if ('click' == $data['type']) {
                ...
                $data['url'] = $this->wechatHelperService->add_url_suffix($data['url'], ['wechat_ru_id' => $this->ru_id]);
                $data['key'] = '';
    ...
    public function menu_del()
    {
        ...
        $id = request()->input('id', 0);
        if (empty($id)) {
            return $this->message(lang('admin/wechat.menu_select_del'), null, 2, true);
        }

        $menuInfo = $this->wechatManageService->wechatMenuInfo($id);

        if (empty($menuInfo)) {
            return $this->message(lang('admin/wechat.menu_not_exit'), null, 2, true);
        }
        ...
        return $this->message(lang('admin/wechat.drop') . lang('admin/common.success'), route('seller/wechat/menu_list'), 2, true);
    }
    ...
    public function sys_menu()
    {
        ...
        if (empty($list)) {
            return $this->message(lang('admin/wechat.menu_empty'), null, 2, true);
        }
        // 转换成微信所需的数组
        /*
        ...
        if (empty($rs)) {
            return $this->message(lang('admin/wechat.errcode') . $this->weObj->errCode . lang('admin/wechat.errmsg') . $this->weObj->errMsg, null, 2, true);
        }
        return $this->message(lang('admin/wechat.menu_create') . lang('admin/common.success'), route('seller/wechat/menu_list'), 2, true);
    }
    ...
    public function subscribe_list()
    {
        ...
        // 分页
        $offset = $this->pageLimit(route('seller/wechat/subscribe_list'), $this->page_num);
    ...
    public function subscribe_search()
    {
        ...
        $offset = $this->pageLimit(route('seller/wechat/subscribe_search', $filter), $this->page_num);
        ...
        $this->assign('page', $this->pageShow($total));
        $this->assign('list', $list);
        return $this->display('seller.wechat.subscribelist');
    }
    ...
    public function sysfans()
    {
        ...
        return redirect()->route('seller/wechat/subscribe_list', ['wechat_id' => $this->wechat_id]);
    }
    ...
    public function subscribe_update()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            $offset = $this->pageLimit(route('seller/wechat/subscribe_update'), $this->page_num);
        ...
        $this->assign('request_url', route('seller/wechat/subscribe_update')); // 请求URL

        $this->assign('persent', 0);
        $this->assign('page_name', lang('admin/wechat.sub_list'));
        return $this->display('seller.wechat.update');
    }
    ...
    public function custom_message_list()
    {
        ...
        $uid = request()->input('uid', 0);
        if (empty($uid)) {
            return $this->message(lang('admin/wechat.select_openid'), null, 2, true);
        }

        // 分页
        $filter['uid'] = $uid;
        $offset = $this->pageLimit(route('seller/wechat/custom_message_list', $filter), $this->page_num);
    ...
    public function sys_tags()
    {
        ...
        return redirect()->route('seller/wechat/subscribe_list');
    }
    ...
    public function user_tag_update()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            $offset = $this->pageLimit(route('seller/wechat/user_tag_update'), $this->page_num);
        ...
        $this->assign('request_url', route('seller/wechat/user_tag_update')); // 请求URL
    ...
    public function tags_delete()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            if (!empty($tag_id)) {
                ...
                return response()->json(['error' => 0, 'msg' => lang('admin/wechat.tag_delete_sucess'), 'url' => route('seller/wechat/subscribe_list')]);
            } else {
    ...
    public function batch_tagging()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            if (!empty($openlist) && is_array($openlist)) {
                ...
                $num = count($openlist);
                if ($num >= 50) {
                    return $this->message(lang('admin/wechat.batch_tagging_limit'), route('seller/wechat/subscribe_list'), 2, true);
                }
                // 微信端打标签
                $rs = $this->weObj->batchtaggingTagsMembers($tag_id, $openlist);
                if (empty($rs)) {
                    return $this->message(lang('admin/wechat.errcode') . $this->weObj->errCode . lang('admin/wechat.errmsg') . $this->weObj->errMsg, route('seller/wechat/subscribe_list'), 2, true);
                }
                // 本地数据处理
                $is_true = 0;
                ...
                if ($is_true == 0) {
                    return $this->message(lang('admin/wechat.tag_move_sucess'), route('seller/wechat/subscribe_list'), true);
                } elseif ($is_true == 1) {
                    return $this->message(lang('admin/wechat.tag_move_fail') . ", " . lang('admin/wechat.tag_move_exit'), route('seller/wechat/subscribe_list'), 2, true);
                } elseif ($is_true == 3) {
                    return $this->message(lang('admin/wechat.tag_move_fail') . ", " . lang('admin/wechat.tag_move_three'), route('seller/wechat/subscribe_list'), 2, true);
                }
            } else {
                return $this->message(lang('admin/wechat.select_please'), null, 2, true);
            }
        }
    }
    ...
    public function qrcode_list()
    {
        ...
        // 分页
        $offset = $this->pageLimit(route('seller/wechat/qrcode_list'), $this->page_num);
    ...
    public function qrcode_edit()
    {
        ...
        if (!empty($id)) {
            ...
            return redirect()->route('seller/wechat/qrcode_list');
        }
    ...
    public function share_list()
    {
        ...
        // 分页
        $offset = $this->pageLimit(route('seller/wechat/share_list'), $this->page_num);
    ...
    public function qrcode_del()
    {
        ...
        $id = request()->input('id', 0);
        if (empty($id)) {
            return $this->message(lang('admin/wechat.select_please') . lang('admin/wechat.qrcode'), null, 2, true);
        }
        // 删除
        $this->wechatManageService->deleteWechatQrcode($this->wechat_id, $id);

        return $this->message(lang('admin/wechat.qrcode') . lang('admin/wechat.drop') . lang('admin/common.success'), route('seller/wechat/qrcode_list'), true);
    }
    ...
    public function article()
    {
        ...
        // 分页
        $offset = $this->pageLimit(route('seller/wechat/article'), $this->page_num);
    ...
    public function article_edit()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            if (!$form->isEmpty($data['title'], 1)) {
                return $this->message(lang('admin/wechat.title') . lang('admin/wechat.empty'), null, 2, true);
            }
            if (!$form->isEmpty($data['content'], 1)) {
                return $this->message(lang('admin/wechat.content') . lang('admin/wechat.empty'), null, 2, true);
            }
            ...
            if ($file && $file->isValid()) {
                ...
                if ($file->getSize() > 2 * 1024 * 1024) {
                    return $this->message(lang('admin/wechat.file_size_limit '), null, 2, true);
                }
                // 验证文件格式
                if (!in_array($file->getClientMimeType(), ['image/jpeg', 'image/png'])) {
                    return $this->message(lang('admin/wechat.not_file_type'), null, 2, true);
                }
                $result = $this->upload('data/attached/article', true);
                if ($result['error'] > 0) {
                    return $this->message($result['message'], null, 2, true);
                }
            ...
            return $this->message(lang('admin/wechat.wechat_editor') . lang('admin/common.success'), route('seller/wechat/article'), true);
        }
    ...
    public function gallery_album()
    {
        ...
        $offset = $this->pageLimit(route('seller/wechat/gallery_album'), $this->page_num);
    ...
    public function article_edit_news()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            if (!empty($article_id) && is_array($article_id)) {
                ...
                return redirect()->route('seller/wechat/article');
            } else {
                return $this->message(lang('admin/wechat.please_add_again'), null, 2, true);
            }
        }
    ...
    public function articles_list()
    {
        ...
        $offset = $this->pageLimit(route('seller/wechat/articles_list'), $this->page_num);
    ...
    public function article_news_del()
    {
        ...
        return redirect()->route('seller/wechat/article_edit_news');
    }
    ...
    public function article_del()
    {
        ...
        $id = request()->input('id', 0);
        if (empty($id)) {
            return $this->message(lang('admin/wechat.select_please') . lang('admin/wechat.article'), null, 2, true);
        }
    ...
    public function picture()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            if ($file && $file->isValid()) {
                ...
                $result = $this->upload('data/attached/article', true);
                if ($result['error'] > 0) {
                    return $this->message($result['message'], route('seller/wechat/picture'), 2, true);
                }
                ...
                if (empty($data['file_name'])) {
                    return $this->message(lang('admin/wechat.please_upload'), null, 2, true);
                }
                ...
                return redirect()->route('seller/wechat/picture');
            }
        }

        // 分页
        $offset = $this->pageLimit(route('seller/wechat/picture'), $this->page_num);
    ...
    public function voice()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            if ($file && $file->isValid()) {
                ...
                if ($result['error'] > 0) {
                    return $this->message($result['message'], route('seller/wechat/voice'), 2, true);
                }
                ...
                return redirect()->route('seller/wechat/voice');
            }
        }

        // 分页
        $offset = $this->pageLimit(route('seller/wechat/voice'), $this->page_num);
    ...
    public function video()
    {
        ...
        // 分页
        $offset = $this->pageLimit(route('seller/wechat/video'), $this->page_num);
    ...
    public function video_edit()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            if (empty($data['file']) || empty($data['file_name']) || empty($data['size'])) {
                return $this->message(lang('admin/wechat.video_empty'), null, 2, true);
            }
            ...
            return $this->message(lang('admin/wechat.upload_video') . lang('admin/common.success'), route('seller/wechat/video'), true);
        }
    ...
    public function media_edit()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            if (!$form->isEmpty($id, 1)) {
                return $this->message(lang('admin/wechat.empty'), null, 2, true);
            }
    ...
    public function download()
    {
        ...
        if (empty($pic)) {
            return $this->message(lang('admin/wechat.file_not_exist'), null, 2, true);
        }
    ...
    public function mass_list()
    {
        ...
        // 分页
        $offset = $this->pageLimit(route('seller/wechat/mass_list'), $this->page_num);
    ...
    public function mass_message()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            if (empty($tag_id) || $tag_id == 0 || empty($media_id)) {
                return $this->message(lang('admin/wechat.please_select_massage'), null, 2, true);
            }
            ...
            if (!empty($article_info['article_id'])) {
                ...
                foreach ($articles as $key => $val) {
                    ...
                    $article[$key]['content_source_url'] = empty($artinfo['link']) ? dsc_url('/#/wechatMedia/' . $artinfo['id']) : strip_tags(html_out($artinfo['link']));
                ...
                $article[0]['content_source_url'] = empty($article_info['link']) ? dsc_url('/#/wechatMedia/' . $article_info['id']) : strip_tags(html_out($article_info['link']));
            ...
            return $this->message(lang('admin/wechat.mass_sending_wait'), route('seller/wechat/mass_message'), true);
        }
        ...
        // 图文信息
        $offset = $this->pageLimit(route('seller/wechat/mass_message'), $this->page_num);
    ...
    public function mass_del()
    {
        ...
        if (empty($msg_id)) {
            return $this->message(lang('admin/wechat.massage_not_exist'), null, 2, true);
        }
        ...
        return redirect()->route('seller/wechat/mass_list');
    }
    ...
    public function auto_reply()
    {
        ...
        if (!empty($type)) {
            ...
            $offset = $this->pageLimit(route('seller/wechat/auto_reply', $filter), $this->page_num);
    ...
    public function reply_subscribe()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            return $this->message(lang('admin/wechat.wechat_editor') . lang('admin/common.success'), route('seller/wechat/reply_subscribe'), true);
        }
        // 关注自动回复数据
    ...
    public function reply_msg()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            return $this->message(lang('admin/wechat.wechat_editor') . lang('admin/common.success'), route('seller/wechat/reply_msg'), true);
    ...
    public function rule_edit()
    {
        if (request()->isMethod('POST')) {
            ...
            if (!$form->isEmpty($data['rule_name'], 1)) {
                return $this->message(lang('admin/wechat.rule_name_empty'), null, 2, true);
            }
            if (!$form->isEmpty($rule_keywords, 1)) {
                return $this->message(lang('admin/wechat.rule_keywords_empty'), null, 2, true);
            }
            if (empty($data['content']) && empty($data['media_id'])) {
                return $this->message(lang('admin/wechat.rule_content_empty'), null, 2, true);
            }
            if (strlen($data['rule_name']) > 60) {
                return $this->message(lang('admin/wechat.rule_name_length_limit'), null, 2, true);
            }
            ...
            foreach ($rule_keywords as $val) {
                ...
                $count = $model->count();
                if ($count >= 1) {
                    return $this->message(lang('admin/wechat.rule_keywords_exit', ['val' => $val]), null, 2, true);
                }
            }
            ...
            return $this->message(lang('admin/wechat.wechat_editor') . lang('admin/common.success'), route('seller/wechat/reply_keywords'), true);
        }
    }
    ...
    public function reply_del()
    {
        ...
        return redirect()->route('seller/wechat/reply_keywords');
    }
    ...
    public function template()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            if (empty($data['primary_industry']) || empty($data['secondary_industry'])) {
                return $this->message(lang('admin/wechat.please_select_industry'), null, 2, true);
            }
            ...
            return $this->message(lang('admin/wechat.wechat_editor') . lang('admin/common.success'), null, true);
        }
    ...
    public function switch_template()
    {
        ...
        if ($status == 1) {
            ...
            if (empty($template['template_id'])) {
                ...
                if ($template_id) {
                    ...
                    return $this->message($this->weObj->errMsg, null, 2, true);
                }
            }
            // 重新启用 更新状态status
            $data['status'] = 1;
        ...
        return redirect()->route('seller/wechat/template');
    }
    ...
    public function share_count()
    {
        ...
        // 分页
        $offset = $this->pageLimit(route('seller/wechat/share_count'), $this->page_num);
    ...
    public function share_count_delete()
    {
        ...
        return $this->message(lang('admin/wechat.drop') . lang('admin/common.success'), route('seller/wechat/share_count'), true);
    }
    ...
    public function extend_index()
    {
        ...
        // 功能扩展列表
        $plugins = $this->wechatManageService->wechatExtendList($this->wechat_id, $this->wechat_type, $this->ru_id);
    ...
    public function extend_edit()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            if (empty($data['keywords'])) {
                return $this->message(lang('admin/wechat.must_keywords'), null, 2, true);
            }
            ...
            if (!empty($this->plugin_name)) {
                ...
                if (!empty($rs)) {
                    ...
                    if (empty($handler) && !empty($rs['enable'])) {
                        return $this->message(lang('admin/wechat.extend_is_enabled'), null, 2, true);
                    } else {
                ...
                return $this->message($msg . lang('admin/common.success'), route('seller/wechat/extend_index'), true);
            }

            return $this->message(lang('admin/common.install') . lang('admin/common.editor') . lang('admin/common.fail'), route('seller/wechat/extend_index'), true);
        }
        ...
        if (!empty($handler)) {
            ...
            // 修改页面显示
            if (empty($info)) {
                return $this->message(lang('admin/wechat.please_select_commond'), null, 2, true);
            }
            $info['config'] = empty($info['config']) ? [] : unserialize($info['config']);
        }

        $extend_config['wechat_ru_id'] = $this->ru_id;
        $extend_config['postion'] = ['ur_here' => L('wechat_extend')]; // 插件当前位置
        $extend_config['seller'] = isset($this->seller) ? $this->seller : '';
        $extend_config['privilege_seller'] = isset($this->privilege_seller) ? $this->privilege_seller : 0;
        $extend_config['menu'] = isset($this->menu) ? $this->menu : '';
        $extend_config['menu_select'] = isset($this->menu_select) ? $this->menu_select : '';

        // 插件实例
        $obj = $this->wechatManageService->wechatPluginInstance($this->plugin_name, $info, $extend_config);
    ...
    public function extend_uninstall()
    {
        ...
        return $this->message(lang('admin/common.uninstall') . lang('admin/common.success'), route('seller/wechat/extend_index'), true);
    }
    ...
    public function winner_list()
    {
        ...
        $offset = $this->pageLimit(route('seller/wechat/winner_list', $filter), $this->page_num);
    ...
    public function winner_issue()
    {
        ...
        if (empty($id)) {
            return $this->message(lang('admin/wechat.please_select_prize'), null, 2, true);
        }
        ...
        if (!empty($cancel)) {
            ...
            return $this->message(lang('admin/wechat.unset_issue_status') . lang('admin/common.success'), route('seller/wechat/winner_list', ['ks' => $activity_type]), true);
        } else {
            $data['issue_status'] = 1;
            $this->wechatManageService->updateWechatPrize($this->wechat_id, $id, $data);

            return $this->message(lang('admin/wechat.set_issue_status') . lang('admin/common.success'), route('seller/wechat/winner_list', ['ks' => $activity_type]), true);
        }
    }
    ...
    public function winner_del()
    {
        ...
        return $this->message(lang('admin/common.delete') . lang('admin/common.success'), route('seller/wechat/winner_list', ['ks' => $activity_type]), true);
    }
    ...
    public function export_winner()
    {
        ...
        if (request()->isMethod('POST')) {
            ...
            if (empty($starttime) || empty($endtime)) {
                return $this->message(lang('admin/wechat.select_start_end_time'), null, 2, true);
            }
            ...
            if ($starttime > $endtime) {
                return $this->message(lang('admin/wechat.start_lt_end_time'), null, 2, true);
            }
            ...
            if (!empty($total_page)) {
                ...
                return $this->message(lang('admin/wechat.data_null'), null, 2, true);
            }
        }

        return redirect()->route('seller/wechat/winner_list', ['ks' => $plugin_name]);
    }
    ...
    public function market_index()
    {
        ...
        if (!empty($markets)) {
            foreach ($markets as $key => $val) {
                $markets[$key]['url'] = route('seller/wechat/market_list', ['type' => $val['keywords']]);

                // 商家过滤不使用的功能
                if ($this->ru_id > 0) {
                    if ($val['keywords'] == 'redpack') {
                        unset($markets[$key]);
                    }
                }
            }
    ...
    public function market_list()
    {
        ...
        $extend_config['postion'] = ['ur_here' => L('wechat_market')]; // 插件当前位置
        $extend_config['seller'] = isset($this->seller) ? $this->seller : '';
        $extend_config['privilege_seller'] = isset($this->privilege_seller) ? $this->privilege_seller : 0;
        $extend_config['menu'] = isset($this->menu) ? $this->menu : '';
        $extend_config['menu_select'] = isset($this->menu_select) ? $this->menu_select : '';
    ...
    public function market_del()
    {
        ...
        return $this->message(lang('admin/wechat.market_delete') . lang('admin/common.success'), route('seller/wechat/market_list', ['type' => $this->market_type]), true);
    }
