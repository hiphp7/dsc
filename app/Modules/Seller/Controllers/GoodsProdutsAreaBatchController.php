...
class GoodsProdutsAreaBatchController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'ajax_insert') {
            ...
            if (session()->has('goods_list') && session('goods_list')) {
                ...
                if (!empty($result['list']['goods_attr'])) {
                    ...
                    //查询数据是否已经存在;
                    $sql = "SELECT product_id FROM " . $GLOBALS['dsc']->table('products_area') . " WHERE goods_id = '" . $result['list']['goods_id'] . "'" .
                    ...
                    if ($product_id) {
                        $this->db->autoExecute($this->dsc->table('products_area'), $other, 'UPDATE', "product_id = '$product_id'");
                        ...
                        if ($GLOBALS['_CFG']['goods_attr_price'] == 1) {
                            ...
                            $this->db->autoExecute($this->dsc->table('goods'), $goods_other, 'UPDATE', "goods_id = '" . $result['list']['goods_id'] . "' AND product_id = '$product_id' AND product_table = 'products_area'");
                        }
                        ...
                        $this->db->autoExecute($this->dsc->table('products_area'), $other, 'INSERT');
        ...
        elseif ($_REQUEST['act'] == 'download') {
            ...
            $goods_id = isset($_REQUEST['goods_id']) ? intval($_REQUEST['goods_id']) : 0;
            $model = isset($_REQUEST['model']) ? intval($_REQUEST['model']) : 0;
            $warehouse_id = isset($_REQUEST['warehouse_id']) ? intval($_REQUEST['warehouse_id']) : 0; //仓库ID ;
            ...
            if ($_GET['charset'] != $GLOBALS['_CFG']['lang']) {
                $lang_file = '../languages/' . $_GET['charset'] . '/' . SELLER_PATH . '/goods_produts_area_batch.php';
            ...
            if (isset($GLOBALS['_LANG']['upload_product'])) {
                ...
                if ($_GET['charset'] == 'zh-CN' || $_GET['charset'] == 'zh-TW') {
                    ...
                    $attr_info = $this->get_list_download($goods_info['goods_sn'], [$region_name, $city_name], $_attribute, count($_attribute), $model);
