    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'ajax_insert') {
            ...
            if (session()->has('goods_list') && session('goods_list')) {
                ...
                if (!empty($result['list']['goods_attr'])) {
                    ...
                    //查询数据是否已经存在;
                    $sql = "SELECT product_id FROM " . $this->dsc->table('products') . " WHERE goods_id = '" . $result['list']['goods_id'] . "'" . $where;
                    $product_id = $this->db->getOne($sql, true);

                    if ($product_id) {
                        $this->db->autoExecute($this->dsc->table('products'), $other, 'UPDATE', "product_id = '$product_id'");
                        ...
                        if ($GLOBALS['_CFG']['goods_attr_price'] == 1) {
                            ...
                            $this->db->autoExecute($this->dsc->table('goods'), $goods_other, 'UPDATE', "goods_id = '" . $result['list']['goods_id'] . "' AND product_id = '$product_id' AND product_table = 'products'");
                        }
                        ...
                        $this->db->autoExecute($this->dsc->table('products'), $other, 'INSERT');
        ...
        elseif ($_REQUEST['act'] == 'download') {
            ...
            if ($_GET['charset'] != $GLOBALS['_CFG']['lang']) {
                $lang_file = '../languages/' . $_GET['charset'] . '/' . SELLER_PATH . '/goods_produts_batch.php';
