...
class GoodsWarehouseAttrBatchController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'upload') {
            ...
            if ($_FILES['file']['name']) {
                $line_number = 0;
                $field_list = array_keys($GLOBALS['_LANG']['upload_warehouse_attr']); // 字段列表
        ...
        elseif ($_REQUEST['act'] == 'ajax_insert') {
            ...
            if (session()->has('goods_list') && session('goods_list')) {
                ...
                $goods_list = $this->dscRepository->pageArray($page_size, $page, $goods_list);
                ...
                //查询数据是否已经存在;
                $sql = "SELECT id FROM " . $this->dsc->table('warehouse_attr') . " WHERE goods_id = '" . $result['list']['goods_id'] . "' AND warehouse_id = '" . $result['list']['warehouse_id'] . "'" .
                ...
                if ($this->db->getOne($sql, true)) {
                    ...
                    $this->db->autoExecute($this->dsc->table('warehouse_attr'), $other, 'UPDATE', "goods_id = '" . $result['list']['goods_id'] . "' AND warehouse_id = '" . $result['list']['warehouse_id'] . "' AND goods_attr_id = '" . $result['list']['goods_attr_id'] . "' $where");
                    $result['status_lang'] = '<span style="color: red;">' . $GLOBALS['_LANG']['updated_data_success'] . '</span>';
                } else {
                    $other['admin_id'] = session('seller_id');

                    $this->db->autoExecute($this->dsc->table('warehouse_attr'), $other, 'INSERT');
        ...
        elseif ($_REQUEST['act'] == 'download') {
            ...
            if ($_GET['charset'] != $GLOBALS['_CFG']['lang']) {
                ...
                if (file_exists($lang_file)) {
                    unset($GLOBALS['_LANG']['upload_warehouse_attr']);
                    require($lang_file);
                }
            }
            if (isset($GLOBALS['_LANG']['upload_warehouse_attr'])) {
                /* 创建字符集转换对象 */
                if ($_GET['charset'] == 'zh-CN' || $_GET['charset'] == 'zh-TW') {
                    $to_charset = $_GET['charset'] == 'zh-CN' ? 'GB2312' : 'BIG5';
                    $data = join(',', $GLOBALS['_LANG']['upload_warehouse_attr']) . "\t\n";
                    ...
                    echo join(',', $GLOBALS['_LANG']['upload_warehouse_attr']);
                }
            } else {
                echo 'error: ' . $GLOBALS['_LANG']['upload_warehouse_attr'] . ' not exists';
            }
        }
    }
    ...
    private function get_goods_attr_list($goods_id)
    {
        ...
        $sql = "SELECT ga.goods_attr_id, ga.goods_id, ga.attr_id, ga.attr_value, ga.attr_sort, ga.admin_id, war.attr_price AS attr_price FROM " . $this->dsc->table("goods_attr") . " AS ga " .
            " LEFT JOIN " . $this->dsc->table("warehouse_attr") . " AS war ON ga.goods_attr_id = war.goods_attr_id AND ga.goods_id = war.goods_id" .
