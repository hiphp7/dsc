...
class MerchantsCustomController extends InitController
{
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            //页面分菜单 by wu start
            $tab_menu = [];
            $tab_menu[] = ['curr' => 0, 'text' => $GLOBALS['_LANG']['07_merchants_window'], 'href' => 'merchants_window.php?act=list'];
            $tab_menu[] = ['curr' => 1, 'text' => $GLOBALS['_LANG']['06_merchants_custom'], 'href' => 'merchants_custom.php?act=list'];
            ...
            $this->smarty->assign('action_link', ['text' => $GLOBALS['_LANG']['add_shop_window'], 'href' => 'merchants_custom.php?act=add', 'class' => 'icon-plus']);
            $this->smarty->assign('full_page', 1);

            $win_list = $this->get_seller_custom($seller_theme);
        ...
        elseif ($_REQUEST['act'] == 'add') {
            ...
            $this->smarty->assign('action_link', ['text' => $GLOBALS['_LANG']['shop_windows_list'], 'href' => 'merchants_custom.php?act=list', 'class' => 'icon-reply']);
            ...
            //检查名称是否重复
            $sql = 'SELECT COUNT(*) FROM ' . $this->dsc->table('seller_shopwindow') . " WHERE win_name = '$_POST[winname]' and ru_id='" . session('ru_id') . "'";
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            /* 删除该橱窗的图标 */
            $sql = "SELECT win_img, win_custom FROM " . $this->dsc->table('seller_shopwindow') . " WHERE id = '$id' and ru_id='" . $adminru['ru_id'] . "'";
            $win = $this->db->getRow($sql);
        ...
        elseif ($_REQUEST['act'] == 'drop_win_goods') {
            ...
            if ($checkboxes) {
                if ($type == 1) {
                    ...
                    $sql = "DELETE FROM " . $this->dsc->table('seller_shopwindow') . " WHERE win_type = 0 AND id in($id)";
                    $this->db->query($sql);
                }
            }
     ...
     * @return  array
     */
    private function get_seller_custom($seller_theme)
    {
        $adminru = get_admin_ru_id();
        $sql = "SELECT * FROM " . $this->dsc->table('seller_shopwindow') . ' WHERE ru_id = \'' . $adminru['ru_id'] . '\' AND seller_theme = \'' . $seller_theme . '\' AND	win_type = 0';

        $win_list = $this->db->getAll($sql);

        foreach ($win_list as $key => $val) {
            $win_list[$key]['seller_theme'] = $GLOBALS['_LANG']['template_mall'] . substr($val['seller_theme'], -1);
