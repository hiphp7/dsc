...
function move_plugin_library($tmp_name, &$msg)
{
    ...
    foreach ($rec as $row) {
        $dir = plugin_path('plugins/' . Str::studly($row['code']) . '/temp');
        if (!file_exists($dir)) {
            make_dir($dir);
        }
        ...
        if (!@rename($source_dir . $row['library'], $target_dir . $row['library'])) {
            if (!@copy(plugin_path('plugins/' . Str::studly($row['code']) . '/temp' . $row['library'], $target_dir . $row['library']))) {
                $return_value = false;
...
function get_editable_libs($curr_template, $curr_page_libs)
{
    ...
    if ($xml_content = @file_get_contents(storage_public('themes/' . $GLOBALS['_CFG']['template'] . '/libs.xml'))) {
