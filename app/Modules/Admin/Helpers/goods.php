...
function goods_list($is_delete = 0, $real_goods = 1, $conditions = '', $review_status = 0, $real_division = 0)
{
    ...
    if ($result === false) {
        ...
        if ($filter['cat_type'] != 'seller' && $filter['cat_id']) {
            ...
            $leftjoin = " LEFT JOIN " . $GLOBALS['dsc']->table('goods_cat') . " AS gc ON g.goods_id = gc.goods_id";
        }
        ...
        if ($filter['cat_type'] != 'seller' && $filter['cat_id']) {
            ...
            $sql = "SELECT COUNT(*) FROM " . $GLOBALS['dsc']->table('goods') . " AS g " .
                $leftjoin .
            ...
            $sql = "SELECT SUM(CASE WHEN " . $goods_where . " THEN 1 ELSE 0 END) FROM " . $GLOBALS['dsc']->table('goods') . " AS g " .
                $leftjoin .
                " WHERE 1";
        ...
        $sql = "SELECT g.goods_id,g.goods_thumb, g.goods_name, g.user_id, g.brand_id, g.goods_type, g.goods_sn, g.shop_price, " .
            "g.is_on_sale, g.is_best, g.is_new, g.is_show, g.is_hot, g.sort_order, g.goods_number, g.integral, g.commission_rate, " .
    ...
    for ($i = 0; $i < $count; $i++) {
        ...
        if (!empty($row[$i]['url']) && (strpos($row[$i]['url'], 'http://') === false && strpos($row[$i]['url'], 'https://') === false && strpos($row[$i]['url'], 'errorImg.png') === false)) {
            $row[$i]['url'] = $GLOBALS['dsc']->url() . $row[$i]['url'];
        }
    }
