    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'view') {
            ...
            if (!$is_multi) {
                ...
                $sql = "SELECT COUNT(*) AS access_count, area FROM " . $this->dsc->table('stats') .
                    " WHERE access_time >= '$start_date' AND access_time < " . ($end_date + 86400) .
                ...
                $sql = "SELECT access_time, area FROM " . $this->dsc->table('stats') .
                    " WHERE $where";
                $res = $this->db->query($sql);
                $area_arr = [];
                foreach ($res as $val) {
            ...
            if (!$is_multi) {
                ...
                $sql = "SELECT COUNT(*) AS access_count, referer_domain FROM " . $this->dsc->table('stats') .
                    " WHERE access_time >= '$start_date' AND access_time <= " . ($end_date + 86400) .
                ...
                foreach ($res as $val) {
                    ...
                    $from_xml .= "<set name='" . str_replace(['http://', 'https://'], ['', ''], $from) . "' value='$val[access_count]' color='" . chart_color($key) . "' />";
                ...
                $sql = "SELECT access_time, referer_domain FROM " . $this->dsc->table('stats') .
                    " WHERE $where";
            ...
            if (!$is_multi) {
                $filename = gmdate($GLOBALS['_CFG']['date_format'], $start_date + $timezone * 3600) . '_' .
                    gmdate($GLOBALS['_CFG']['date_format'], $end_date + $timezone * 3600);
            ...
            return $this->smarty->display('flow_stats.dwt');
        } /* 报表下载 */
        elseif ($act = 'download') {
            ...
            if (EC_CHARSET != 'gbk') {
                echo dsc_iconv(EC_CHARSET, 'gbk', $data) . "\t";
            } else {
                echo $data . "\t";
            }
        }
