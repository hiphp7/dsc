...
class TpApiController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'kdniao') {
            ...
            $this->smarty->assign('ur_here', $GLOBALS['_LANG']['kdniao']);
            $this->smarty->assign('form_act', 'kdniao_update');
            $this->smarty->assign('type', 'kdniao');
            $this->smarty->assign('api_lang', $GLOBALS['_LANG']['tpApi']['kdniao']);

            $api_config = [];
            $api_config['client_id'] = $this->config['kdniao_client_id'];
            $api_config['appkey'] = $this->config['kdniao_appkey'];
            $api_config['kdniao_account_use'] = $this->config['kdniao_account_use'];
            $this->smarty->assign('api_config', $api_config);

            return $this->smarty->display('tp_api.dwt');
        } //快递鸟更新
        elseif ($_REQUEST['act'] == 'kdniao_update') {
            ...
            $sql = " UPDATE " . $this->dsc->table('shop_config') . " SET value = '$client_id' WHERE code = 'kdniao_client_id' ";
            $this->db->query($sql);
            $sql = " UPDATE " . $this->dsc->table('shop_config') . " SET value = '$appkey' WHERE code = 'kdniao_appkey' ";
            $this->db->query($sql);

            $sql = " UPDATE " . $GLOBALS['dsc']->table('shop_config') . " SET value = '$kdniao_account_use' WHERE code = 'kdniao_account_use' ";
            ...
            $link[] = ['text' => $GLOBALS['_LANG']['go_back'], 'href' => 'tp_api.php?act=kdniao'];
            return sys_msg($GLOBALS['_LANG']['save_success'], 0, $link);
        } //快递鸟打印
        elseif ($_REQUEST['act'] == 'kdniao_print') {
            ...
            //判断订单
            if (empty($order_ids)) {
                return sys_msg($GLOBALS['_LANG']['not_select_order'], 1, $link);
            }
            ...
            if (count($shipping_ids) > 1) {
                return sys_msg($GLOBALS['_LANG']['select_express_order_print'], 1, $link);
            }
            ...
            if ($order_ids && $order_ids[0]) {
                ...
                foreach ($order_ids as $order_id) {
                    ...
                    if ($result["ResultCode"] != "100") {
                        $batch_error[] = $GLOBALS['_LANG']['order'] . "（{$order_id}）：" . $GLOBALS['_LANG']['electron_order_fail'] . "：{$result['Reason']}";
                        continue;
                    }
                    ...
                    if (!empty($result['PrintTemplate'])) {
                        ...
                        $batch_error[] = $GLOBALS['_LANG']['order'] . "（" . $order_id . "）：" . $GLOBALS['_LANG']['not_print_template'];
                        continue;
                    }
        ...
        elseif ($_REQUEST['act'] == 'order_print') {
            ...
            $order_print_logo = asset('assets') . '/' . $this->config['order_print_logo'];
            ...
            /* 模板赋值 */
            //$this->smarty->assign('ur_here', $GLOBALS['_LANG']['order_print']);
