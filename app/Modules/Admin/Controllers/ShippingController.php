...
class ShippingController extends InitController
{
    ...
    {
        $exc = new Exchange($this->dsc->table('shipping'), $this->db, 'shipping_id', 'shipping_name');
        ...
        elseif ($_REQUEST['act'] == 'install') {
            ...
            /* 检查该配送方式是否已经安装 */
            $sql = "SELECT shipping_id FROM " . $this->dsc->table('shipping') . " WHERE shipping_code = '$code'";
            $id = $this->db->GetOne($sql);

            if ($id > 0) {
                /* 该配送方式已经安装过, 将该配送方式的状态设置为 enable */
                $this->db->query("UPDATE " . $this->dsc->table('shipping') . " SET enabled = 1 WHERE shipping_code = '$code' LIMIT 1");
            } else {
                $shipping_name = Str::studly($code);
                ...
                if (file_exists($modules)) {
                    ...
                    $sql = "INSERT INTO " . $this->dsc->table('shipping') . " (" .
                        "shipping_code, shipping_name, shipping_desc, insure, support_cod, enabled, print_bg, config_lable, print_model" .
                        ") VALUES (" .
                        "'" . addslashes($modules['code']) . "', '" . addslashes($GLOBALS['_LANG'][$modules['code']]) . "', '" .
                        addslashes($GLOBALS['_LANG'][$modules['desc']]) . "', '$insure', '" . intval($modules['cod']) . "', 1, '" . addslashes($modules['print_bg']) . "', '" . addslashes($modules['config_lable']) . "', '" . $modules['print_model'] . "')";
                    $this->db->query($sql);
                }
            }

            /* 记录管理员操作 */
            admin_log(addslashes($GLOBALS['_LANG'][$modules['code']]), 'install', 'shipping');

            /* 提示信息 */
            $lnk[] = ['text' => $GLOBALS['_LANG']['go_back'], 'href' => 'shipping.php?act=list'];
            return sys_msg(sprintf($GLOBALS['_LANG']['install_succeess'], $GLOBALS['_LANG'][$modules['code']]), 0, $lnk);
        }
        ...
        elseif ($_REQUEST['act'] == 'uninstall') {
            ...
            /* 获得该配送方式的ID */
            $row = $this->db->GetRow("SELECT shipping_id, shipping_name, print_bg FROM " . $this->dsc->table('shipping') . " WHERE shipping_code='$code'");
            $shipping_id = $row['shipping_id'];
            $shipping_name = $row['shipping_name'];

            /* 删除 shipping_fee 以及 shipping 表中的数据 */
            if ($row) {
                $all = $this->db->getCol("SELECT shipping_area_id FROM " . $this->dsc->table('shipping_area') . " WHERE shipping_id='$shipping_id'");
                if (!empty($all)) {
                    $in = db_create_in(join(',', $all));
                    $this->db->query("DELETE FROM " . $this->dsc->table('area_region') . " WHERE shipping_area_id $in");
                }

                $this->db->query("DELETE FROM " . $this->dsc->table('shipping_area') . " WHERE shipping_id='$shipping_id'");
                $this->db->query("DELETE FROM " . $this->dsc->table('shipping') . " WHERE shipping_id='$shipping_id'");

                $this->db->query("DELETE FROM " . $this->dsc->table('goods_transport_express') . " WHERE shipping_id='$shipping_id'");
                $this->db->query("DELETE FROM " . $this->dsc->table('goods_transport_tpl') . " WHERE shipping_id='$shipping_id'");
        ...
        elseif ($_REQUEST['act'] == 'print_index') {
            ...
            if ($row) {
                ...
                $ship_tpl['print_bg'] = empty($ship_tpl['print_bg']) ? '' : get_image_path($ship_tpl['print_bg']);
            }
            $this->smarty->assign('shipping', $ship_tpl);
            $this->smarty->assign('shipping_id', $shipping_id);
            $this->smarty->assign('pint_url', get_image_path('data/print/pint.swf'));
        ...
        elseif ($_REQUEST['act'] == 'recovery_default_template') {
            ...
            if ($code) {
                ...
                if (file_exists($modules)) {
                    ...
                    /* 恢复默认 */
                    $this->db->query("UPDATE " . $this->dsc->table('shipping_tpl') . " SET print_bg = '" . addslashes($modules['print_bg']) . "',  config_lable = '" . addslashes($modules['config_lable']) . "' " .
                        " WHERE shipping_id = '$shipping_id' and ru_id='" . $adminru['ru_id'] . "' LIMIT 1");
                }
            }
        ...
        elseif ($_REQUEST['act'] == 'print_upload') {
            ...
            if ($res) {
                echo '<script language="javascript">';
                echo 'parent.call_flash("bg_add", "' . get_image_path($src) . '");';
                echo '</script>';
            }
        }
