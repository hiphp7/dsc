...
class SuppliersGoodsController extends InitController
{
    ...
    public function index()
    {
        ...
        elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit' || $_REQUEST['act'] == 'copy') {
            ...
            if ($is_add) {
                ...
                if (isset($ecscpCookie['last_choose']) && !empty($ecscpCookie['last_choose'])) {
                    $last_choose = explode('|', $ecscpCookie['last_choose']);
                }
                $goods = array(
                    'goods_id' => 0,
                    'goods_desc' => '',
                ...
                $sql = "DELETE FROM " . $this->dsc->table('group_goods') .
                    " WHERE parent_id = 0 AND admin_id = '$admin_id'";
                $this->db->query($sql);

                /* 关联文章 */
                $goods_article_list = array();
                $sql = "DELETE FROM " . $this->dsc->table('goods_article') .
                    " WHERE goods_id = 0 AND admin_id = '$admin_id'";
                $this->db->query($sql);
                ...
                /* 商品信息 */
                $sql = "SELECT * FROM " . $this->dsc->table('goods') . " WHERE goods_id = '$_REQUEST[goods_id]'";
                ...
                if ($_REQUEST['act'] == 'copy') {
                    ...
                    $sql = "SELECT '0' AS goods_id, link_goods_id, is_double, '$admin_id' AS admin_id" .
                    ...
                    $sql = "SELECT goods_id, '0' AS link_goods_id, is_double, '$admin_id' AS admin_id" .
                        " FROM " . $this->dsc->table('link_goods') .
                    ...
                    $sql = "SELECT 0 AS parent_id, goods_id, goods_price, '$admin_id' AS admin_id " .
                    ...
                    $sql = "SELECT 0 AS goods_id, article_id, '$admin_id' AS admin_id " .
                ...
                $sql = "SELECT cat_id FROM " . $this->dsc->table('goods_cat') . " WHERE goods_id = '$_REQUEST[goods_id]'";
                $goods['other_cat'] = $this->db->getCol($sql);
                foreach ($goods['other_cat'] as $cat_id) {
                    $other_cat_list[$cat_id] = $this->categoryService->catList($cat_id);
                }
                ...
                /* 图片列表 */
                $sql = "SELECT * FROM " . $this->dsc->table('goods_gallery') . " WHERE goods_id = '$goods[goods_id]'";
            ...
            $cat_list = $this->categoryService->catList($goods['cat_id']);
            $this->smarty->assign('cat_list', $cat_list);
            $this->smarty->assign('brand_list', get_brand_list());
            ...
            if (isset($_REQUEST['goods_id'])) {
                $volume_price_list = $this->goodsCommonService->getVolumePriceList(intval($_REQUEST['goods_id']));
            }
            ...
            /* 显示商品信息页面 */
            return $this->smarty->display('goods_info.htm');
        }
        ...
        elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update') {
            ...
            if ($is_insert) {
                if ($code == '') {
                    $sql = "INSERT INTO " . $this->dsc->table('goods') . " (goods_name, goods_name_style, goods_sn, " .
                ...
                $sql = "UPDATE " . $this->dsc->table('goods') . " SET " .
            ...
            if ((isset($_POST['attr_id_list']) && isset($_POST['attr_value_list'])) || (empty($_POST['attr_id_list']) && empty($_POST['attr_value_list']))) {
                ...
                $sql = "SELECT * FROM " . $this->dsc->table('goods_attr') . " WHERE goods_id = '$goods_id'";
                ...
                $sql = "UPDATE " . $this->dsc->table('goods') . " SET keywords = '$keywords' WHERE goods_id = '$goods_id' LIMIT 1";
                ...
                foreach ($goods_attr_list as $attr_id => $attr_value_list) {
                    foreach ($attr_value_list as $attr_value => $info) {
                        if ($info['sign'] == 'insert') {
                            $sql = "INSERT INTO " . $this->dsc->table('goods_attr') . " (attr_id, goods_id, attr_value, attr_price)" .
                                "VALUES ('$attr_id', '$goods_id', '$attr_value', '$info[attr_price]')";
                        } elseif ($info['sign'] == 'update') {
                            $sql = "UPDATE " . $this->dsc->table('goods_attr') . " SET attr_price = '$info[attr_price]' WHERE goods_attr_id = '$info[goods_attr_id]' LIMIT 1";
                        } else {
            ...
            if ($goods_img !== false) {
                $this->db->query("UPDATE " . $this->dsc->table('goods') . " SET goods_img = '$goods_img' WHERE goods_id='$goods_id'");
            }

            if ($original_img !== false) {
                $this->db->query("UPDATE " . $this->dsc->table('goods') . " SET original_img = '$original_img' WHERE goods_id='$goods_id'");
            }

            if ($goods_thumb !== false) {
                $this->db->query("UPDATE " . $this->dsc->table('goods') . " SET goods_thumb = '$goods_thumb' WHERE goods_id='$goods_id'");
            }
            ...
            if (!$is_insert && isset($_POST['old_img_desc'])) {
                foreach ($_POST['old_img_desc'] as $img_id => $img_desc) {
                    $sql = "UPDATE " . $this->dsc->table('goods_gallery') . " SET img_desc = '$img_desc' WHERE img_id = '$img_id' LIMIT 1";
                    $this->db->query($sql);
                }
            }
            ...
            if ($proc_thumb && !$GLOBALS['_CFG']['retain_original_img'] && !empty($original_img)) {
                $this->db->query("UPDATE " . $this->dsc->table('goods') . " SET original_img='' WHERE `goods_id`='{$goods_id}'");
                $this->db->query("UPDATE " . $this->dsc->table('goods_gallery') . " SET img_original='' WHERE `goods_id`='{$goods_id}'");
                @unlink('../' . $original_img);
                @unlink('../' . $img);
            }

            /* 记录上一次选择的分类和品牌 */
            cookie()->queue('ECSCP[last_choose]', $catgory_id . '|' . $brand_id, gmtime() + 86400);
        ...
        elseif ($_REQUEST['act'] == 'show_image') {
            ...
            return $this->smarty->display('goods_show_image.htm');
        }
        ...
        elseif ($_REQUEST['act'] == 'drop_image') {
            ...
            /* 删除图片文件 */
            $sql = "SELECT img_url, thumb_url, img_original " .
                " FROM " . $GLOBALS['dsc']->table('goods_gallery') .
                " WHERE img_id = '$img_id'";
            ...
            /* 删除数据 */
            $sql = "DELETE FROM " . $GLOBALS['dsc']->table('goods_gallery') . " WHERE img_id = '$img_id' LIMIT 1";
        ...
        elseif ($_REQUEST['act'] == 'add_link_goods') {
            ...
            $linked_array = dsc_decode($_GET['add_ids']);
            $linked_goods = dsc_decode($_GET['JSON']);
            ...
            foreach ($linked_array as $val) {
                if ($is_double) {
                    ...
                    $sql = "INSERT INTO " . $this->dsc->table('link_goods') . " (goods_id, link_goods_id, is_double, admin_id) " .
                        "VALUES ('$val', '$goods_id', '$is_double', '$admin_id')";
                    $this->db->query($sql, 'SILENT');
                }

                $sql = "INSERT INTO " . $this->dsc->table('link_goods') . " (goods_id, link_goods_id, is_double, admin_id) " .
                    "VALUES ('$goods_id', '$val', '$is_double', '$admin_id')";
                $this->db->query($sql, 'SILENT');
            }
        ...
        elseif ($_REQUEST['act'] == 'drop_link_goods') {
            ...
            $drop_goods = dsc_decode($_GET['drop_ids']);
        ...
        elseif ($_REQUEST['act'] == 'add_group_goods') {
            ...
            $fittings = dsc_decode($_GET['add_ids']);
            $arguments = dsc_decode($_GET['JSON']);
            $goods_id = $arguments[0];
            $price = $arguments[1];

            foreach ($fittings as $val) {
                $sql = "INSERT INTO " . $this->dsc->table('group_goods') . " (parent_id, goods_id, goods_price, admin_id) " .
                    "VALUES ('$goods_id', '$val', '$price', '$admin_id')";
                $this->db->query($sql, 'SILENT');
            }
        ...
        elseif ($_REQUEST['act'] == 'drop_group_goods') {
            ...
            $fittings = dsc_decode($_GET['drop_ids']);
            $arguments = dsc_decode($_GET['JSON']);
            $goods_id = $arguments[0];
            $price = $arguments[1];
        ...
        elseif ($_REQUEST['act'] == 'get_article_list') {
            $filters = (array)dsc_decode(json_str_iconv($_GET['JSON']));
        ...
        elseif ($_REQUEST['act'] == 'drop_goods_article') {
            ...
            $articles = dsc_decode($_GET['drop_ids']);
            $arguments = dsc_decode($_GET['JSON']);
            $goods_id = $arguments[0];

            $sql = "DELETE FROM " . $this->dsc->table('goods_article') . " WHERE " . db_create_in($articles, "article_id");
            $this->db->query($sql);
    ...
    {
        $sql = "DELETE FROM " . $GLOBALS['dsc']->table('volume_price') .
        ...
        foreach ($price_list as $key => $price) {
            ...
            if (!empty($price)) {
                $sql = "INSERT INTO " . $GLOBALS['dsc']->table('volume_price') .
