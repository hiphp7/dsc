...
use App\Services\Merchant\MerchantsWindowManageService;
...
class MerchantsWindowController extends InitController
{
    ...
    protected $merchantsWindowManageService;

    public function __construct(
        BaseRepository $baseRepository,
        MerchantsWindowManageService $merchantsWindowManageService
    )
    {
        $this->baseRepository = $baseRepository;
        $this->merchantsWindowManageService = $merchantsWindowManageService;
    }
    ...
    public function index()
    {
        ...
        if ($shop_id < 1) {
            $lnk[] = ['text' => lang('admin/merchants_window.set_shop'), 'href' => 'index.php?act=merchants_first'];
            return sys_msg(lang('admin/merchants_window.not_set_shop'), 0, $lnk);
        }
        ...
        if ($_REQUEST['act'] == 'list') {
            ...
            $this->smarty->assign('ur_here', lang('admin/merchants_window.window_list'));
            $this->smarty->assign('action_link', ['text' => lang('admin/merchants_window.add_window'), 'href' => 'merchants_window.php?act=add']);
            $this->smarty->assign('full_page', 1);

            $win_list = $this->merchantsWindowManageService->getSellerWindow($seller_theme);
        ...
        elseif ($_REQUEST['act'] == 'add') {
            ...
            $this->smarty->assign('ur_here', lang('admin/merchants_window.add_window'));
            $this->smarty->assign('action_link', ['text' => lang('admin/merchants_window.window_list'), 'href' => 'merchants_window.php?act=list']);
            $this->smarty->assign('form_action', 'insert');

            $res = SellerShopinfo::where('ru_id', $adminru['ru_id']);
            $seller_win = $this->baseRepository->getToArrayFirst($res);

            $seller_win['win_type'] = 1;

            $this->smarty->assign('seller_win', $seller_win);

            $type_list = $this->merchantsWindowManageService->winGoodsTypeList($seller_shopinfo['win_goods_type']);
            ...
            if ($number > 0) {
                return sys_msg(sprintf(lang('admin/merchants_window.window_name_exist'), stripslashes($_POST['winname'])), 1);
            }
            ...
            if ($win_name) {
                ...
                $link[0]['text'] = lang('admin/merchants_window.continue_add');
                $link[0]['href'] = 'merchants_window.php?act=add';

                $link[1]['text'] = lang('admin/merchants_window.back_list');
                $link[1]['href'] = 'merchants_window.php?act=list';

                $link[2]['text'] = lang('admin/merchants_window.add_window_goods');
                $link[2]['href'] = 'merchants_window.php?act=add_win_goods&id=' . $id;

                return sys_msg(lang('admin/merchants_window.add_window_success'), 0, $link);
            } else {
                $link[0]['text'] = lang('admin/merchants_window.continue_add');
                $link[0]['href'] = 'merchants_window.php?act=add';

                return sys_msg(lang('admin/merchants_window.add_window_fail'), 0, $link);
            }
        }
        ...
        elseif ($_REQUEST['act'] == 'edit') {
            ...
            $this->smarty->assign('ur_here', lang('admin/merchants_window.edit_window'));
            $this->smarty->assign('action_link', ['text' => lang('admin/merchants_window.window_list'), 'href' => 'merchants_window.php?act=list']);
            $this->smarty->assign('seller_win', $seller_win);
            ...
            $is_only = SellerShopwindow::where('win_name', $win_name)->where('id', '<>', $id)->count();
            if ($is_only < 1) {
                $param['win_name'] = $win_name;
            }
            $res = SellerShopwindow::where('id', $id)->update($param);
            if ($res >= 0) {
                /* 清除缓存 */
                clear_cache_files();
                ...
                $link[0]['text'] = lang('admin/merchants_window.back_list');
                $link[0]['href'] = 'merchants_window.php?act=list';
                return sys_msg(lang('admin/merchants_window.edit_window_success'), 0, $link);
            } else {
        ...
        elseif ($_REQUEST['act'] == 'edit_win_name') {
            ...
            /* 检查名称是否重复 */
            $number = SellerShopwindow::where('win_name', $name)
                ->where('ru_id', session('ru_id'))
                ->count();
            if ($number > 0) {
                return make_json_error(sprintf(lang('admin/merchants_window.window_exist'), $name));
            } else {
                $data = ['win_name' => $name];
                ...
                if ($res >= 0) {
                    ...
                    return make_json_result(sprintf(lang('admin/merchants_window.edit_fail'), $name));
                }
            }
        }
        /*------------------------------------------------------ */
        ...
        elseif ($_REQUEST['act'] == 'edit_sort_order') {
            ...
            if ($res >= 0) {
                ...
                return make_json_error(sprintf(lang('admin/merchants_window.edit_fail'), $name));
            }
        }
        ...
        elseif ($_REQUEST['act'] == 'remove') {
            ...
            /* 删除该橱窗的图标 */
            $win_img = SellerShopwindow::where('id', $id)
                ->where('ru_id', $adminru['ru_id'])
                ->value('win_img');
            $win_img = $win_img ? $win_img : '';
            if (!empty($win_img)) {
                //@unlink(storage_public(DATA_DIR .$nav_img));
                @unlink($win_img);
            }
        ...
        elseif ($_REQUEST['act'] == 'add_win_goods') {
            ...
            $win_goods = $this->merchantsWindowManageService->getWinGoods($id);

            if ($win_goods == 'no_cc') {
                return sys_msg(lang('admin/merchants_window.no_access'));
            } else {
                $win_info = $this->merchantsWindowManageService->getWinInfo($id);

                $this->smarty->assign('win_info', $win_info);

                $this->smarty->assign('ur_here', lang('admin/merchants_window.add_window_goods'));
        ...
        elseif ($_REQUEST['act'] == 'update_win_goods') {
            ...
            $link[1]['text'] = lang('admin/merchants_window.back_list');
            $link[1]['href'] = 'merchants_window.php?act=list';
            return sys_msg(lang('admin/merchants_window.edit_window_goods_success'), 0, $link);
        }
        ...
        elseif ($_REQUEST['act'] == 'drop_win_goods') {
            ...
            if ($checkboxes) {
                $id = $this->baseRepository->getExplode($checkboxes);
                SellerShopwindow::where('win_type', 1)->whereIn('id', $id)->delete();
            }

            $link[0]['text'] = lang('admin/merchants_window.back_shop_list');
            $link[0]['href'] = 'merchants_window.php?act=list';

            return sys_msg(lang('admin/merchants_window.delete_success'), 0, $link);
        }
    }
}
