...
use App\Services\Goods\GoodsWarehouseAttrManageService;
...
class GoodsWarehouseAttrController extends InitController
{
    ...
    protected $goodsWarehouseAttrManageService;
    protected $goodsAttrService;

    public function __construct(
        BaseRepository $baseRepository,
        GoodsWarehouseAttrManageService $goodsWarehouseAttrManageService,
        GoodsAttrService $goodsAttrService
    )
    {
        $this->baseRepository = $baseRepository;
        $this->goodsWarehouseAttrManageService = $goodsWarehouseAttrManageService;
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'warehouse_list') {
            ...
            $warehouse_list = $this->goodsWarehouseAttrManageService->warehouseProductList($goods_id);
            ...
            return $this->smarty->display('goods_warehouse_attr_list.htm');
        }
        ...
        elseif ($_REQUEST['act'] == 'query') {
            $warehouse_list = $this->goodsWarehouseAttrManageService->warehouseProductList(session('warehouse_goods_id'));
        ...
        elseif ($_REQUEST['act'] == 'product_list') {
            ...
            $region_name = RegionWarehouse::where('region_id', $warehouse_id)->value('region_name');
            ...
            /* 获取商品规格列表 */
            $attribute = $this->goodsWarehouseAttrManageService->getGoodsSpecificationsList($goods_id);
            if (empty($attribute)) {
            ...
            /* 取商品的货品 */
            $product = $this->goodsWarehouseAttrManageService->productWarehouseList($goods_id, $warehouse_id);
        ...
        elseif ($_REQUEST['act'] == 'edit_product_sn') {
            ...
            if ($this->goodsWarehouseAttrManageService->checkProductWarehouseSnExist($product_sn, $product_id, $adminru['ru_id'])) {
                return make_json_error($GLOBALS['_LANG']['sys']['wrong'] . $GLOBALS['_LANG']['exist_same_product_sn']);
            }

            /* 修改 */
            $data = ['product_sn' => $product_sn];
            $result = ProductsWarehouse::where('product_id', $product_id)->update($data);
            if ($result) {
                clear_cache_files();
        ...
        elseif ($_REQUEST['act'] == 'edit_bar_code') {
            ...
            if ($this->goodsWarehouseAttrManageService->checkProductWarehouseSnExist($bar_code, $product_id, $adminru['ru_id'], 1)) {
        ...
        elseif ($_REQUEST['act'] == 'edit_product_number') {
            ...
            /* 货品库存 */
            $product = $this->goodsWarehouseAttrManageService->getProductWarehouseInfo($product_id, 'product_number, warehouse_id, goods_id');
        ...
        elseif ($_REQUEST['act'] == 'product_remove') {
            ...
            /* 删除货品 */
            $result = ProductsWarehouse::where('product_id', $product_id)->delete();
            if ($result) {
        ...
        elseif ($_REQUEST['act'] == 'product_add_execute') {
            ...
            /* 判断是否为初次添加 */
            $insert = true;
            if ($this->goodsWarehouseAttrManageService->productWarehouseNumberCount($product['goods_id'], '', $product['warehouse_id']) > 0) {
                $insert = false;
            }

            /* 取出商品信息 */
            $res = Goods::where('goods_id', $product['goods_id']);
            $goods = $this->baseRepository->getToArrayFirst($res);
            if (empty($goods)) {
                return sys_msg($GLOBALS['_LANG']['sys']['wrong'] . $GLOBALS['_LANG']['cannot_found_goods'], 1, [], false);
            }
            ...
            foreach ($product['product_sn'] as $key => $value) {
                ...
                $goods_attr_id = $this->goodsWarehouseAttrManageService->handleGoodsAttr($product['goods_id'], $id_list, $is_spec_list, $value_price_list);

                /* 是否为重复规格的货品 */
                $goods_attr = $this->goodsAttrService->sortGoodsAttrIdArray($goods_attr_id);
                $goods_attr = implode('|', $goods_attr['sort']);
                if ($this->goodsWarehouseAttrManageService->checkGoodsAttrExist($goods_attr, $product['goods_id'], 0, $product['warehouse_id'])) {
                    continue;
                }
                ...
                if (!empty($value)) {
                    ...
                    if ($this->goodsWarehouseAttrManageService->checkGoodsSnExist($value)) {
                        continue;
                    }
                    if ($this->goodsWarehouseAttrManageService->checkProductSnExist($value)) {
                        continue;
                    }
                }
                ...
                $insert_id = ProductsWarehouse::insertGetId($data);
                if ($insert_id < 1) {
                    continue;
                }
                ...
                if (empty($value)) {
                    $data = [
                        'product_sn' => $goods['goods_sn'] . "g_p" . $insert_id
                    ];
                    ProductsWarehouse::where('product_id', $insert_id)->update($data);
                }

                /* 修改商品表库存 */
                $product_count = $this->goodsWarehouseAttrManageService->productWarehouseNumberCount($product['goods_id'], '', $product['warehouse_id']);
                if ($this->goodsWarehouseAttrManageService->updateWarehouseGoods($product['goods_id'], 'goods_number', $product_count)) {
                    //记录日志
