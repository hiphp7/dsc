...
use App\Services\Goods\GoodsProdutsAreaBatchManageService;
use App\Services\Merchant\MerchantCommonService;

/**
 * 商品批量上传、修改
 */
class GoodsProdutsAreaBatchController extends InitController
{
    protected $dscRepository;
    protected $merchantCommonService;
    protected $baseRepository;
    protected $goodsProdutsAreaBatchManageService;

    public function __construct(
        DscRepository $dscRepository,
        MerchantCommonService $merchantCommonService,
        BaseRepository $baseRepository,
        GoodsProdutsAreaBatchManageService $goodsProdutsAreaBatchManageService
    )
    {
        $this->dscRepository = $dscRepository;
        $this->merchantCommonService = $merchantCommonService;
        $this->baseRepository = $baseRepository;
        $this->goodsProdutsAreaBatchManageService = $goodsProdutsAreaBatchManageService;
    }
    ...
    public function index()
    {
        ...
        if ($_REQUEST['act'] == 'add') {
            ...
            $warehouse_id = isset($_REQUEST['warehouse_id']) && !empty($_REQUEST['warehouse_id']) ? intval($_REQUEST['warehouse_id']) : 0; //仓库
            $area_id = isset($_REQUEST['area_id']) && !empty($_REQUEST['area_id']) ? intval($_REQUEST['area_id']) : 0; //地区
            $city_id = isset($_REQUEST['city_id']) && !empty($_REQUEST['city_id']) ? intval($_REQUEST['city_id']) : 0; //城市
            ...
            $attribute_list = $this->goodsProdutsAreaBatchManageService->getAttributeList($goods_id);
            ...
            return $this->smarty->display('goods_produts_area_batch.dwt');
        }
        ...
        elseif ($_REQUEST['act'] == 'upload') {
            ...
            return $this->smarty->display('goods_produts_area_batch_add.dwt');
        }
        /* ------------------------------------------------------ */
        //-- 动态添加数据�        �库;
        /* ------------------------------------------------------ */
            ...
            if (session()->has('goods_list') && session('goods_list')) {
                ...
                if (!empty($result['list']['goods_attr'])) {
                    ...
                    $product_id = $res->where('goods_id', $result['list']['goods_id'])
                        ...
                        ->where('city_id', $result['list']['city_id'])
                        ->value('product_id');
                    $product_id = $product_id ? $product_id : 0;

                    if ($product_id) {
                        ProductsArea::where('product_id', $product_id)->update($other);
                        if ($GLOBALS['_CFG']['goods_attr_price'] == 1) {
                            $goods_other = [
                            ...
                            Goods::where('goods_id', $result['list']['goods_id'])
                                ...
                                ->where('product_table', 'products_area')
                                ->update($goods_other);
                        }
                        ...
                        $product_id = ProductsArea::insertGetId($other);
        ...
        elseif ($_REQUEST['act'] == 'download') {
            ...
            $goods_id = isset($_REQUEST['goods_id']) ? intval($_REQUEST['goods_id']) : 0;
            $model = isset($_REQUEST['model']) ? intval($_REQUEST['model']) : 0;
            $warehouse_id = isset($_REQUEST['warehouse_id']) && !empty($_REQUEST['warehouse_id']) ? intval($_REQUEST['warehouse_id']) : 0; //仓库ID ;
            $area_id = isset($_REQUEST['area_id']) && !empty($_REQUEST['area_id']) ? intval($_REQUEST['area_id']) : 0; //当前地区ID;
            $goods_attr = isset($_REQUEST['goods_attr']) && !empty($_REQUEST['goods_attr']) ? explode(',', $_REQUEST['goods_attr']) : [];
            $city_id = isset($_REQUEST['city_id']) && !empty($_REQUEST['city_id']) ? intval($_REQUEST['city_id']) : 0; //当前地区城市;

            // 文件标签
            // Header("Content-type: application/octet-stream");
            header("Content-type: application/vnd.ms-excel; charset=utf-8");
            Header("Content-Disposition: attachment; filename=goods_produts_area_batch" . $goods_id . "_" . "$warehouse_id" . "_" . "$area_id" . "_" . "$city_id" . ".csv");
            ...
            if (isset($GLOBALS['_LANG']['upload_product'])) {
                ...
                if ($_GET['charset'] == 'zh-CN' || $_GET['charset'] == 'zh-TW') {
                    ...
                    $city_name = get_table_date('region_warehouse', "region_id = '$city_id'", ['region_name'], 2);

                    $attr_info = $this->goodsProdutsAreaBatchManageService->getListDownload($goods_info['goods_sn'], [$region_name, $city_name], $_attribute, count($_attribute), $model);
