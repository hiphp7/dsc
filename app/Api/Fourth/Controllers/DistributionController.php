...
use App\Api\Fourth\Transformers\DistributionTransformer;
...
class DistributionController extends Controller
{
    ...
    public function application(Request $request, RightsCardService $rightsCardService, CommonRepository $commonRepository)
    {
        ...
        $membership_card_id_repeat = $request->input('membership_card_id_repeat', 0); // 重新申请权益卡id
        ...
        $type = 2;
        $card_receive_value = $rightsCardService->cardReceiveValue($type, $membership_card_id_repeat);
    ...
    public function drpcard(Request $request, RightsCardService $rightsCardService)
    {
        ...
        // 续费权益卡id
        $membership_card_id_renew = $request->input('membership_card_id_renew', 0);
        // 重新申请权益卡id
        $membership_card_id_repeat = $request->input('membership_card_id_repeat', 0);

        $result['list'] = $rightsCardService->cardList($type, $receive_type, $user_id, $membership_card_id_renew, $membership_card_id_repeat);

        if ($receive_type == 'order' || $receive_type == 'integral' || $receive_type == 'buy' || $receive_type == 'free') {
            $drp_config = $this->drpService->drpConfig();
    ...
    public function apply(Request $request, RightsCardService $rightsCardService, DistributeService $distributeService, CommonRepository $commonRepository)
    {
        ...
        // 权益卡领取类型 goods,购买商品 order 消费金额 integral,消费积分 buy 指定金额购买、free 免费领取
        ...
        if (empty($repeat) && empty($shop_info)) {
            ...
            $drp_shop['type'] = 0;
            $drp_shop['apply_channel'] = $distributeService->transformReceiveType($receive_type);
            ...
            // 重新申请成功提示
            $msg = lang('drp.drp_shop_apply_again_success');

            if ($shop_info['membership_card_id'] == $membership_card_id) {
                return $this->succeed(['error' => 1, 'msg' => lang('drp.drp_shop_apply_again_same')]);
            }
        ...
        $parent_id = $commonRepository->getDrpShopAffiliate($user_id);
     ...
     * @return JsonResponse
     */
    public function renew(Request $request, RightsCardService $rightsCardService, DistributeService $distributeService)
    {
        $this->validate($request, [
            'receive_type' => 'required|string',
            'membership_card_id' => 'required|integer'
        ...
        // 成功提示信息
        $msg = lang('drp.drp_shop_renew_success');
        ...
        if ($res) {
            ...
            $account_log = [
                'amount' => $amount,
                'pay_points' => $pay_point,
                'user_note' => lang('admin/drpcard.receive_type_' . $receive_type) . '(' . lang('drp.drp_shop_renew') . ')',
                'receive_type' => $receive_type, // 权益卡领取类型
                'membership_card_id' => $membership_card_id,
                'log_id' => $log_id, // 日志id
    ...
    public function shop(Request $request)
    {
        ...
        if ($shop_info) {
            $result['shop_info'] = $shop_info;
            // 未过期判断审核状态
            if ($shop_info['membership_status'] == 1 && $shop_info['audit'] != 1) {
                $result['error'] = 1;
                $result['msg'] = lang('drp.drp_shop_audit');
            }
     ...
     * @return JsonResponse
     */
    public function index(Request $request, RightsCardService $rightsCardService, DistributeService $distributeService)
    {
        $this->validate($request, []);
        ...
        if ($shop_info) {
            ...
            // 转换领取类型
            $shop_info['receive_type_format'] = $distributeService->transformApplyChannel($shop_info['apply_channel']);

            if (!empty($result['membership_card_info'])) {
                // 会员权益卡 过期时间判断
                $expiry = $rightsCardService->checkExpiryTime($result['membership_card_info'], $shop_info);
                ...
                if (isset($expiry['expiry_status']) && $expiry['expiry_status'] == 1) {
                    ...
                    $distributeService->editDrpShop($user_id, $data);
                    $rightsCardService->restoreUsersRank($user_id);
                }

                $expiry['expiry_time_format'] = $expiry['expiry_time_format'] ?? '';
