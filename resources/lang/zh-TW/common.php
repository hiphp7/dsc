<?php

$_LANG['merchants_article'] = "您查看的文章已不存在，請與網站聯繫";

/* 優惠券 start */
$_LANG['lang_goods_coupons']['all_pay'] = "全場券";
$_LANG['lang_goods_coupons']['user_pay'] = "會員券";
$_LANG['lang_goods_coupons']['goods_pay'] = "購物券";
$_LANG['lang_goods_coupons']['reg_pay'] = "註冊券";
$_LANG['lang_goods_coupons']['free_pay'] = "免郵券";
$_LANG['lang_goods_coupons']['not_pay'] = "未知";
$_LANG['lang_goods_coupons']['is_goods'] = "限商品";
$_LANG['lang_goods_coupons']['is_cate'] = "限分類";
$_LANG['lang_goods_coupons']['is_all'] = "全品類通用";

$_LANG['lang_coupons_receive_failure'] = "領取失敗,券已經被領完了";
$_LANG['lang_coupons_user_receive'] = "領取失敗,您已經領取過該券了!每人限領取 %s 張";
$_LANG['lang_coupons_user_rank'] = "領取失敗,僅限會員等級為: %s 領取";
$_LANG['lang_coupons_receive_succeed'] = "領取成功！感謝您的參與，祝您購物愉快~";

$_LANG['man'] = "滿";
$_LANG['available_full'] = "元可用";
$_LANG['valid_time'] = '有效期';
$_LANG['zhi'] = "至";
$_LANG['receive_now'] = "立即領取";
$_LANG['follow_and_receive'] = "關注店鋪並領取";
/* 優惠券 end */

/* 眾籌 start */
$_LANG['lang_crowd_preheat'] = "預熱中";
$_LANG['lang_crowd_of'] = "眾籌中";
$_LANG['lang_crowd_succeed'] = "眾籌成功";

$_LANG['lang_crowd_art_succeed'] = "發布成功";
$_LANG['lang_crowd_art_succeed_repeat'] = "已經發布過啦，請不要重複發布！";
$_LANG['lang_crowd_login'] = "登陸後發布話題";

$_LANG['lang_crowd_page_title'] = "眾籌-收貨地址";

$_LANG['lang_crowd_login_focus'] = "只有登陸用戶才能關注";
$_LANG['lang_crowd_focus_succeed'] = "關注成功";
$_LANG['lang_crowd_focus_repeat'] = "已經關注過啦，請不要重複關注！";
$_LANG['lang_crowd_focus_cancel'] = "取消關注成功!";

$_LANG['lang_crowd_like'] = "點贊成功";
$_LANG['lang_crowd_like_repeat'] = "已經點過贊啦，請不要重複點贊！";

$_LANG['lang_crowd_next_step'] = "下一步";
$_LANG['lang_crowd_not_login'] = "您尚未登錄，請登錄您的賬號！";

$_LANG['lang_crowd_not_pay'] = "您有未支付的眾籌訂單，請付款後再提交新訂單";
$_LANG['lang_crowd_not_address'] = "您沒有設置收貨地址";
/* 眾籌 end */

$_LANG['lang_product_sn'] = "貨品貨號";

$_LANG['dwt_shop_name'] = "";

$_LANG['order_payed_sms'] = '訂單 %s 付款了，收貨人：%s 電話：%s'; //wang

//ecmoban模板堂 --zhuo start 審核收貨人地址
$_LANG['order_address_stay'] = "無法提交訂單<br/>收貨地址已被修改，待審核中...";
$_LANG['order_address_no'] = "無法提交訂單<br/>收貨地址已被修改，審核未通過...";
$_LANG['index_lnk'] = "返回首頁，繼續購物！";

$_LANG['edit_address_success'] = '您的收貨地址信息已成功更新,待審核通過';
$_LANG['address_list_lnk'] = '返回地址列表';

$_LANG['signin_failed_user'] = '收貨地址為空，請您添加收貨地址';
$_LANG['lnk_user'] = "去添加收貨地址";
//ecmoban模板堂 --zhuo end 審核收貨人地址

$_LANG['single_user'] = '用戶曬單'; //by zhang
$_LANG['discuss_user'] = '網友討論圈'; //by zhang
$_LANG['allcount'] = '全部帖子'; //by zhang
$_LANG['s_count'] = '曬單帖';
$_LANG['t_count'] = '討論帖';
$_LANG['w_count'] = '問答帖';
$_LANG['q_count'] = '圈子帖';
$_LANG['reply_number'] = '回複數';
$_LANG['click_count'] = '點擊數';
$_LANG['sort']['single_sort'] = '排序';

$_LANG['set_gcolor'] = '設置商品顏色'; //by zhang

//ecmoban模板堂 --zhuo start
$_LANG['ff'][FF_NOMAINTENANCE] = '未維修';
$_LANG['ff'][FF_MAINTENANCE] = '已維修';
$_LANG['ff'][FF_NOEXCHANGE] = '未換貨';
$_LANG['ff'][FF_EXCHANGE] = '已換貨';
$_LANG['ff'][FF_NOREFOUND] = '未退款';
$_LANG['ff'][FF_REFOUND] = '已退款';


$_LANG['steps_UserLogin'] = '您尚未登錄，無法申請入駐';
$_LANG['UserLogin'] = '去登錄';

$_LANG['please_select'] = '請選擇';
$_LANG['country'] = '國家';
$_LANG['province'] = '省';
$_LANG['city'] = '市';
$_LANG['area'] = '區';

$_LANG['delivery_warehouse'] = '發貨倉庫';

$_LANG['group_shortage'] = "對不起，該套餐主件商品已經庫存不足暫停銷售。\n你現在要進行缺貨登記來預訂該商品嗎？";
$_LANG['group_not_on_sale'] = '對不起，該套餐主件商品已經下架。';
$_LANG['group_goods_not_exists'] = "對不起，指定的套餐主件商品不存在";

$_LANG['not_start'] = '未開始';
$_LANG['overdue'] = '已過期';
$_LANG['has_ended'] = '已結束';
$_LANG['not_use'] = '未使用';
$_LANG['had_use'] = '已使用';
$_LANG['expired'] = '已失效';
$_LANG['completed'] = '已完成';
$_LANG['has_been_snatched'] = '已搶完';
//ecmoban模板堂 --zhuo end

/* 用戶登錄語言項 */
$_LANG['empty_username_password'] = '對不起，您必須完整填寫用戶名和密碼。';
$_LANG['shot_message'] = "短消息";

/* 公共語言項 */

$_LANG['largess'] = '贈品';
$_LANG['sys_msg'] = '系統提示';
$_LANG['catalog'] = '目錄';
$_LANG['please_view_order_detail'] = '商品已發貨，詳情請到用戶中心訂單詳情查看';
$_LANG['user_center'] = '用戶中心';
$_LANG['shop_closed'] = "本店盤點中，請您稍後再來...";
$_LANG['shop_register_closed'] = '該網店暫停註冊';
$_LANG['shop_upgrade'] = "本店升級中，管理員從 <a href=\\\"admin/\\\">管理中心</a> 登錄後，系統會自動完成升級";

$_LANG['process_request'] = '正在處理您的請求...';
$_LANG['please_waiting'] = '請稍等, 正在載入中...';
$_LANG['icp_number'] = 'ICP備案證書號';
$_LANG['plugins_not_found'] = "插件 %s 無法定位";
$_LANG['home'] = '首頁';
$_LANG['back_up_page'] = '返回上一頁';
$_LANG['back_list'] = '返回列表';
$_LANG['illegal_operate'] = '非法操作';
$_LANG['close_window'] = '關閉窗口';
$_LANG['back_home'] = '返回首頁';
$_LANG['ur_here'] = '當前位置:';
$_LANG['all_goods'] = '全部商品';
$_LANG['all_recommend'] = "全部推薦";
$_LANG['all_attribute'] = "全部";
$_LANG['promotion_goods'] = '促銷商品';
$_LANG['best_goods'] = '精品推薦';
$_LANG['new_goods'] = '新品上市';
$_LANG['hot_goods'] = '熱銷商品';
$_LANG['hot_rankings'] = '熱銷排行';
$_LANG['view_cart'] = "查看購物車";
$_LANG['catalog'] = '所有分類';
$_LANG['regist_login'] = '註冊/登錄';
$_LANG['account_login'] = '賬號登錄';
$_LANG['quick_login'] = '掃碼登錄';
$_LANG['personal'] = '個人';
$_LANG['profile'] = '個人資料';
$_LANG['query_info'] = "共執行 %d 個查詢，用時 %f 秒，在線 %d 人";
$_LANG['gzip_enabled'] = '，Gzip 已啟用';
$_LANG['gzip_disabled'] = '，Gzip 已禁用';
$_LANG['memory_info'] = '，佔用內存 %0.3f MB';
$_LANG['cart_info'] = "%d";
$_LANG['shopping_and_other'] = '購買過此商品的人還購買過';
$_LANG['bought_notes'] = '購買記錄';
$_LANG['later_bought_amounts'] = '近期成交數量';
$_LANG['bought_time'] = '購買時間';
$_LANG['turnover'] = '成交';
$_LANG['no_notes'] = '還沒有人購買過此商品';
$_LANG['shop_price'] = "商 城 價";
$_LANG['market_price'] = "市場價";
$_LANG['goods_brief'] = '商品描述：';
$_LANG['goods_album'] = '商品相冊';
$_LANG['promote_price'] = "促 銷 價";
$_LANG['fittings_price'] = '配件價格：';
$_LANG['collect'] = '加入收藏夾';
$_LANG['add_to_cart'] = "加入購物車";
$_LANG['return_to_cart'] = "放回購物車";
$_LANG['search_goods'] = '商品搜索';
$_LANG['search'] = '搜索';
$_LANG['seller_search'] = '店內搜索';
$_LANG['wholesale_search'] = '搜索批發商品';
$_LANG['article_title'] = '文章標題';
$_LANG['article_author'] = '作者';
$_LANG['article_add_time'] = '添加日期';
$_LANG['relative_file'] = '[ 相關下載 ]';
$_LANG['category'] = '分類';
$_LANG['category_icon'] = '分類圖標';
$_LANG['brand'] = '品牌';
$_LANG['price_min'] = '最小价格';
$_LANG['price_max'] = '最大價格';
$_LANG['goods_name'] = '商品名稱';
$_LANG['goods_attr'] = '商品屬性';
$_LANG['goods_price_ladder'] = '價格階梯';
$_LANG['ladder_price'] = '批發價格';
$_LANG['shop_prices'] = "本 店 價";
$_LANG['market_prices'] = "市 場 價";
$_LANG['import_tax'] = "預估稅金";
$_LANG['estimate'] = "預計";
$_LANG['group_buy_price'] = "團 購 價";
$_LANG['seckill_price'] = "秒 殺 價";
$_LANG['presale_price'] = "預 售 價";
$_LANG['deposit'] = '團購保證金';
$_LANG['amount'] = '商品總價';
$_LANG['number'] = '購買數量';
$_LANG['handle'] = '操作';
$_LANG['add'] = '添加';
$_LANG['edit'] = '編輯';
$_LANG['drop'] = '刪除';
$_LANG['view'] = '查看';
$_LANG['modify'] = '修改';
$_LANG['set'] = '設置';
$_LANG['amend_amount'] = '修改數量';
$_LANG['end'] = '結束';
$_LANG['require_field'] = '(必填)';
$_LANG['search_result'] = '搜索結果';
$_LANG['order_number'] = '訂單號';
$_LANG['consignment'] = '發貨單';
$_LANG['activities'] = '商品正在進行的活動';
$_LANG['remark_package'] = '超值禮包';
$_LANG['old_price'] = '原  價：';
$_LANG['package_price'] = '禮包價：';
$_LANG['then_old_price'] = '節  省：';
$_LANG['free_goods'] = '免運費商品';
$_LANG['back_auction_home'] = '返回拍賣首頁';
$_LANG['not_img'] = "暫無圖片";
$_LANG['matters_need_attention'] = "注意事項";
$_LANG['setup_batch'] = "起批";
$_LANG['batches'] = "起批量";
$_LANG['volume'] = "成交量";
$_LANG['contain_content'] = '包含內容';
$_LANG['act_id'] = '活動編號';
$_LANG['bid_time'] = '出價時間';
$_LANG['bid_price'] = '出價價格';
$_LANG['package_already'] = '禮包已下架';
$_LANG['review_comments'] = '查看評論';
$_LANG['snatch_desc'] = '奪寶詳情';

$_LANG['searchkeywords_notice'] = '匹配多個關鍵字全部，可用 "空格" 或 "AND" 連接。如 win32 AND unix<br />匹配多個關鍵字其中部分，可用"+"或 "OR" 連接。如 win32 OR unix';
$_LANG['hidden_outstock'] = '隱藏已脫銷的商品';
$_LANG['keywords'] = '關鍵字';
$_LANG['sc_ds'] = '搜索簡介';
$_LANG['button_search'] = '立即搜索';
$_LANG['no_search_result'] = '無法搜索到您要找的商品！';
$_LANG['all_category'] = '所有分類';
$_LANG['all_brand'] = '所有品牌';
$_LANG['all_option'] = '請選擇';
$_LANG['extension'] = '擴展選項';
$_LANG['gram'] = '克';
$_LANG['kilogram'] = '千克';
$_LANG['goods_sn'] = '商品貨號：';
$_LANG['bar_code'] = '條形條碼';
$_LANG['goods_brand'] = '品　　牌：';
$_LANG['goods_weight'] = '商品重量：';
$_LANG['goods_number'] = '商品庫存：';
$_LANG['goods_give_integral'] = '購買此商品贈送：';
$_LANG['goods_integral'] = '可　　用：';
$_LANG['goods_bonus'] = '購買此商品可獲得紅包：';
$_LANG['goods_free_shipping'] = '此商品為免運費商品，計算配送金額時將不計入配送費用';
$_LANG['goods_rank'] = '用戶評價：';
$_LANG['goods_compare'] = '商品比較';
$_LANG['properties'] = '商品屬性：';
$_LANG['brief'] = '簡要介紹：';
$_LANG['add_time'] = '上架時間：';
$_LANG['residual_time'] = '剩餘時間';
$_LANG['begin_time_soon'] = '距開始時間';
$_LANG['day'] = '天';
$_LANG['hour'] = '小時';
$_LANG['minute'] = '分鐘';
$_LANG['compare'] = '比較';
$_LANG['residual'] = '剩餘';
$_LANG['volume_price'] = '購買商品達到以下數量區間時可享受的優惠價格';
$_LANG['number_to'] = '數量';
$_LANG['article_list'] = '文章列表';
$_LANG['look_and_see'] = '看了又看';
$_LANG['evaluation_single'] = '商品評價';
$_LANG['delivery_speed'] = '發貨速度';
$_LANG['service_attitude'] = '服務態度';
$_LANG['certificate_info'] = '證照信息';
$_LANG['wechat_scan_shop'] = '微信掃碼查看移動端店鋪';
$_LANG['not'] = "無";
$_LANG['open'] = "展開";
$_LANG['open_more'] = "展開更多";
$_LANG['stop'] = "收起";
$_LANG['stop_more'] = "收起更多";
$_LANG['time'] = "時間";
$_LANG['money_symbol'] = "￥";
$_LANG['current_price'] = '當前價格';
$_LANG['start_time'] = '開始時間';
$_LANG['end_time'] = '結束時間';
$_LANG['coming_end'] = '即將結束';
$_LANG['con_cus_service'] = '聯繫客服';
$_LANG['min_fare'] = '最低加價';
$_LANG['max_fare'] = '最高加價';
$_LANG['store_shop'] = '商家店鋪';
$_LANG['see_more'] = '查看更多';
$_LANG['comprehensive'] = '綜合好評';
$_LANG['branch'] = '分';
$_LANG['score_detail'] = '評分明細';
$_LANG['industry_compare'] = '行業相比';
$_LANG['seller_score'] = '店鋪評分';
$_LANG['synthesize'] = '綜合';
$_LANG['goods'] = '商品';
$_LANG['service'] = '服務';
$_LANG['deliver_goods'] = '發貨';
$_LANG['prescription'] = '時效';
$_LANG['store_total'] = '店鋪總分';
$_LANG['company'] = '公&nbsp;&nbsp;&nbsp;&nbsp;司';
$_LANG['seat_of'] = '所在地';
$_LANG['online_service'] = '在線客服';
$_LANG['enter_the_shop'] = '進入店鋪';
$_LANG['follow'] = '關注';
$_LANG['follow_yes'] = '已關注';
$_LANG['follow_goods'] = '關注商品';
$_LANG['follow_store'] = '關注店鋪';
$_LANG['follow_number'] = '關注人數';
$_LANG['assign'] = '確定';
$_LANG['is_cancel'] = '取消';
$_LANG['au_number'] = '次出價';
$_LANG['brand_home'] = '品牌首頁';
$_LANG['brand_category'] = '品牌分類';
$_LANG['brand_special_area'] = '品牌專區';
$_LANG['main_brand'] = '主營品牌';
$_LANG['brand_all'] = '全部品牌';
$_LANG['returnTop'] = '返回頂部';
$_LANG['current'] = '當前';
$_LANG['all_comments'] = '全部評論';
$_LANG['publish_reply'] = '發表回復';
$_LANG['click_getinfo'] = '點擊進入';

$_LANG['upload_success'] = '上傳成功';
$_LANG['upload_fail'] = '上傳失敗';
$_LANG['unknown_error'] = "未知錯誤，請重試";

$_LANG['not_interface_power'] = "暫無該介面許可權";
$_LANG['secret_key_not_null'] = "密鑰不能為空";
$_LANG['illegal_entrance'] = "非法入口";
$_LANG['please_login_member'] = "請登錄會員";

$_LANG['autograph_fail'] = "簽名失敗";
$_LANG['does_not_exist'] = "插件不存在";
$_LANG['not_date_back'] = "無數據返回";

$_LANG['sort_parameter'] = "排好序的參數:";
$_LANG['code_url'] = "請求的url:";
$_LANG['code_json'] = "返回的json數據:<br>\r\n";
$_LANG['code_value'] = "<br>\r\n取出showapi_res_code的值:<br>\r\n";

$_LANG['look_new'] = '找新品';
$_LANG['look_hot'] = '找熱賣';
$_LANG['all_category'] = '全部分類';
$_LANG['change_a_lot'] = '換一批';
$_LANG['best'] = '精品';
$_LANG['see_all'] = '查看全部';
$_LANG['ren'] = '人';
$_LANG['jian_goods'] = '件商品';
$_LANG['guess_love'] = '猜你喜歡';
$_LANG['sale_amount'] = '銷售量';
$_LANG['screen_price'] = '請填寫篩選價格';
$_LANG['screen_price_left'] = '請填寫篩選左邊價格';
$_LANG['screen_price_right'] = '請填寫篩選右邊價格';
$_LANG['screen_price_dy'] = '左邊價格不能大於或等於右邊價格';
$_LANG['publish_top'] = '發表新話題';
$_LANG['types'] = '類型';
$_LANG['publish'] = '發表';
$_LANG['statement'] = '聲明';
$_LANG['statement_one'] = '1、歡迎您在此提出與商品有關的問題，為保證話題質量，鐵牌及以上級別用戶可以發表話題，註冊及以上級別用戶可以回復；';
$_LANG['statement_two'] = '2、網友討論採用先發布後審核原則，如果我們認為您的發貼不能給其他用戶帶來幫助或違反國家有關規定有權刪除';
$_LANG['commentTitle_not'] = '主題不能為空';
$_LANG['commentTitle_xz'] = '標題長度只能在2-50個字元之間';
$_LANG['content_not'] = '內容不能為空';
$_LANG['captcha_not'] = '驗證碼不能為空';
$_LANG['captcha_xz'] = '請輸入4位數的驗證碼';
$_LANG['message_see'] = '【新消息】請查收!';
$_LANG['message_not'] = '【　　　】請查收!';
$_LANG['After_sale_service'] = '發貨並提供售後服務。';

/* 商品比較JS語言項 */
$_LANG['compare_js']['button_compare'] = '比較選定商品';
$_LANG['compare_js']['exist'] = '您已經選擇了%s';
$_LANG['compare_js']['count_limit'] = '最多只能選擇4個商品進行對比';
$_LANG['compare_js']['goods_type_different'] = '\"%s\"和已選擇商品類型不同無法進行對比';

$_LANG['bonus'] = '優惠券：';
$_LANG['no_comments'] = '暫時還沒有任何用戶評論';
$_LANG['give_comments_rank'] = '給出';
$_LANG['comments_rank'] = '評價';
$_LANG['comment_num'] = "用戶評論 %d 條記錄";
$_LANG['login_please'] = '由於您還沒有登錄，因此您還不能使用該功能。';
$_LANG['collect_existed'] = '該商品已經存在於您的收藏夾中。';
$_LANG['collect_success'] = '該商品已經成功地加入收藏夾。';
$_LANG['collect_brand_existed'] = '您已經關注過該品牌';
$_LANG['collect_brand_success'] = '成功關注該品牌';
$_LANG['cancel_brand_success'] = '已取消關注該品牌';
$_LANG['send_authemail_success'] = '驗證郵件發送成功';
$_LANG['copyright'] = "&copy; 2005-%s %s 版權所有，並保留所有權利。";
$_LANG['no_ads_id'] = '沒有指定廣告的ID以及跳轉的URL地址!';
$_LANG['remove_collection_confirm'] = '您確定要從收藏夾中刪除選定的商品嗎？';
$_LANG['err_change_attr'] = '沒有找到指定的商品或者沒有找到指定的商品屬性。';

$_LANG['collect_goods'] = '收藏商品';
$_LANG['plus'] = '加';
$_LANG['minus'] = '減';
$_LANG['yes'] = '是';
$_LANG['no'] = '否';

$_LANG['same_attrbiute_goods'] = '相同%s的商品';

/* TAG */
$_LANG['button_submit_tag'] = '添加我的標記';
$_LANG['tag_exists'] = '您已經為該商品添加過一個標記，請不要重複提交.';
$_LANG['tag_cloud'] = '標籤雲';
$_LANG['tag_anonymous'] = '對不起，只有註冊會員並且正常登錄以後才能提交標記。';
$_LANG['tag_cloud_desc'] = '標籤雲（Tag cloud）是用以表示一個網站中的內容標籤。 標籤（tag、關鍵詞）是一種更為靈活、有趣的商品分類方式，您可以為每個商品添加一個或多個標籤，那麼可以通過點擊這個標籤查看商品其他會員提交的與您的標籤一樣的商品,能夠讓您使用最快的方式查找某一個標籤的所有網店商品。比方說點擊「紅色」這個標籤，就可以打開這樣的一個頁面，顯示所有的以「紅色」 為標籤的網店商品';

/* AJAX 相關 */
$_LANG['invalid_captcha'] = '對不起，您輸入的驗證碼不正確。';
$_LANG['goods_exists'] = "對不起，您的購物車中已經存在相同的商品。";
$_LANG['fitting_goods_exists'] = "對不起，您的購物車中已經添加了該配件。";
$_LANG['invalid_number'] = '對不起，您輸入了一個非法的商品數量。';
$_LANG['not_on_sale'] = '對不起，該商品已經下架。';
$_LANG['no_basic_goods'] = "對不起，您希望將該商品做為配件購買，可是購物車中還沒有該商品的基本件。";
$_LANG['cannt_alone_sale'] = '對不起，該商品不能單獨銷售。';
$_LANG['shortage'] = "對不起，該商品已經庫存不足暫停銷售。\n你現在要進行缺貨登記來預訂該商品嗎？";
$_LANG['shortage_little'] = "該商品已經庫存不足。已將您的購貨數量修改為 %d。\n您現在要去購物車嗎？";
$_LANG['oos_tips'] = '該商品已經庫存不足。您現在要進行缺貨登記嗎？';

$_LANG['addto_cart_success_1'] = "該商品已添加到購物車，您現在還需要繼續購物嗎？\n如果您希望馬上結算，請點擊「確定」按鈕。\n如果您希望繼續購物，請點擊「取消」按鈕。";
$_LANG['addto_cart_success_2'] = "該商品已添加到購物車，您現在還需要繼續購物嗎？\n如果您希望繼續購物，請點擊「確定」按鈕。\n如果您希望馬上結算，請點擊「取消」按鈕。";
$_LANG['no_keywords'] = "請輸入搜索關鍵詞！";

/* 分頁排序 */
$_LANG['exchange_sort']['goods_id'] = '按上架時間排序';
$_LANG['exchange_sort']['exchange_integral'] = '按積分排序';
$_LANG['exchange_sort']['last_update'] = '按更新時間排序';
$_LANG['order'] = [];
$_LANG['order']['DESC'] = '倒序';
$_LANG['order']['ASC'] = '正序';
$_LANG['pager_1'] = '總計 ';
$_LANG['pager_2'] = ' 個記錄';
$_LANG['pager_3'] = '，共 ';
$_LANG['pager_4'] = ' 頁。';
$_LANG['page_first'] = '第一頁';
$_LANG['page_prev'] = '上一頁';
$_LANG['page_next'] = '下一頁';
$_LANG['page_last'] = '最末頁';
$_LANG['btn_display'] = '顯示方式';
$_LANG['page_last_new'] = '末頁';

/* 投票 */
$_LANG['vote_times'] = '參與人次';
$_LANG['vote_ip_same'] = '對不起，您已經投過票了!';
$_LANG['submit_vote'] = '投票';
$_LANG['submit_reset'] = '重選';
$_LANG['vote_success'] = '恭喜你，投票成功';

/* 評論 */
$_LANG['cmt_submit_done'] = '您的帖子已成功發表, 感謝您的參與!';
$_LANG['cmt_submit_wait'] = "您的帖子已成功發表, 請等待管理員的審核!";
$_LANG['cmt_lang']['cmt_empty_username'] = '請輸入您的用戶名稱';
$_LANG['cmt_lang']['cmt_empty_email'] = '請輸入您的電子郵件地址';
$_LANG['cmt_lang']['cmt_error_login'] = '請您登錄後再發表評論';
$_LANG['cmt_lang']['cmt_error_email'] = '電子郵件地址格式不正確';
$_LANG['cmt_lang']['cmt_empty_content'] = '您沒有輸入評論的內容';
$_LANG['cmt_spam_warning'] = '您至少在30秒後才可以繼續發表評論!';
$_LANG['cmt_lang']['captcha_not_null'] = '驗證碼不能為空!';
$_LANG['cmt_lang']['cmt_invalid_comments'] = '無效的評論內容!';
$_LANG['invalid_comments'] = '無效的評論內容!';
$_LANG['error_email'] = '電子郵件地址格式不正確!';
$_LANG['admin_username'] = "管理員：";
$_LANG['reply_comment'] = '回復';
$_LANG['comment_captcha'] = '驗證碼';
$_LANG['comment_login'] = '只有註冊會員才能發表評論，請您登錄後再發表評論';
$_LANG['comment_custom'] = "評論失敗。只有在本店購買過商品的註冊會員才能發表評論。";
$_LANG['comment_brought'] = '評論失敗。只有購買過此商品的註冊用戶才能評論該商品。';
$_LANG['anonymous'] = '匿名用戶';

/* 商品比較 */
$_LANG['compare_remove'] = '移除';
$_LANG['compare_no_goods'] = '您沒有選定任何需要比較的商品或者比較的商品數少於 2 個。';

$_LANG['no_user_name'] = '該用戶名不存在';
$_LANG['undifine_rank'] = '暫無';
$_LANG['not_login'] = '您還沒有登陸';
$_LANG['half_info'] = '信息不全，請填寫所有信息';
$_LANG['no_id'] = '沒有商品ID';
$_LANG['save_success'] = '修改成功';
$_LANG['drop_consignee_confirm'] = '您確定要刪除該收貨人信息嗎？';

/* 奪寶奇兵 */
$_LANG['snatch_js']['price_not_null'] = '價格不能為空';
$_LANG['snatch_js']['price_not_number'] = '價格只能是數字';
$_LANG['snatch_list'] = '奪寶奇兵列表';
$_LANG['not_in_range'] = '你只能在%d到%d之間出價';
$_LANG['also_bid'] = '你已經出過價格 %s 了';
$_LANG['lack_pay_points'] = '你積分不夠，不能出價';
$_LANG['snatch'] = '奪寶奇兵';
$_LANG['snatch_is_end'] = '活動已經結束';
$_LANG['snatch_start_time'] = '本次活動從 %s 到 %s 截止';
$_LANG['price_extent'] = '出價範圍為';
$_LANG['user_to_use_up'] = '用戶可多次出價，每次消耗';
$_LANG['snatch_victory_desc'] = '當本期活動截止時，系統將從所有競價獎品的用戶中，選出在所有競價中出價最低、且沒有其他出價與該價格重複的用戶（即最低且唯一競價），成為該款獎品的獲勝者.';
$_LANG['price_less_victory'] = '如果用戶獲勝的價格低於';
$_LANG['price_than_victory'] = '將能按當期競拍價購得該款獎品；如果用戶獲勝的價格高於';
$_LANG['or_can'] = '則能以';
$_LANG['shopping_product'] = '購買該款獎品';
$_LANG['victory_price_product'] = '獲勝用戶將能按當期競拍價購得該款獎品.';
$_LANG['now_not_snatch'] = '當前沒有活動';
$_LANG['my_integral'] = '我的積分';
$_LANG['bid'] = '出價';
$_LANG['me_bid'] = '我要出價';
$_LANG['me_now_bid'] = '我的出價';
$_LANG['only_price'] = '唯一價格';
$_LANG['view_snatch_result'] = '活動結果';
$_LANG['victory_user'] = '獲獎用戶';
$_LANG['price_bid'] = '所出價格';
$_LANG['bid_time'] = '出價時間';
$_LANG['not_victory_user'] = '沒有獲獎用戶';
$_LANG['snatch_log'] = '參加奪寶奇兵%s ';
$_LANG['not_for_you'] = '你不是獲勝者，不能購買';
$_LANG['order_placed'] = '您已經下過訂單了，如果您想重新購買，請先取消原來的訂單';

/* 購物流程中的前台部分 */
$_LANG['select_spe'] = '請選擇商品屬性';

/* 購物流程中的訂單部分 */
$_LANG['price'] = '價格';
$_LANG['name'] = '名稱';
$_LANG['describe'] = '描述';
$_LANG['fee'] = '費用';
$_LANG['free_money'] = '免費額度';
$_LANG['img'] = '圖片';
$_LANG['no_pack'] = '不要包裝';
$_LANG['no_card'] = '不要賀卡';
$_LANG['bless_note'] = '祝福語';
$_LANG['use_integral'] = '使用積分';
$_LANG['can_use_integral'] = '您當前的可用積分為';
$_LANG['noworder_can_integral'] = '本訂單最多可以使用';
$_LANG['use_surplus'] = '使用餘額';
$_LANG['your_surplus'] = '您當前的可用餘額為';
$_LANG['pay_fee'] = '支付手續費';
$_LANG['insure_fee'] = '保價費用';
$_LANG['need_insure'] = '配送是否需要保價';
$_LANG['cod'] = '配送決定';

$_LANG['curr_stauts'] = '當前狀態';
$_LANG['use_bonus'] = '使用紅包';
$_LANG['use_value_card'] = '使用儲值卡';
$_LANG['value_card_dis'] = '儲值卡折扣';
$_LANG['use_bonus_kill'] = '使用線下紅包';
$_LANG['invoice'] = '開發票';
$_LANG['invoice_type'] = '發票類型';
$_LANG['invoice_title'] = '發票抬頭';
$_LANG['invoice_info'] = '發票信息';
$_LANG['invoice_desc'] = '發票詳情';
$_LANG['invoice_content'] = '發票內容';
$_LANG['order_postscript'] = '訂單附言';
$_LANG['booking_process'] = '缺貨處理';
$_LANG['complete_acquisition'] = '該訂單完成後，您將獲得';
$_LANG['with_price'] = '以及價值';
$_LANG['de'] = '的';
$_LANG['rise'] = '抬頭';
$_LANG['bonus'] = '紅包';
$_LANG['goods_all_price'] = '商品總價';
$_LANG['dis_amount'] = '商品優惠';
$_LANG['discount'] = '折扣';
$_LANG['tax'] = '發票稅額';
$_LANG['shipping_fee'] = '配送費用';
$_LANG['pack_fee'] = '包裝費用';
$_LANG['card_fee'] = '賀卡費用';
$_LANG['total_fee'] = '應付款金額';
$_LANG['self_site'] = '本站';
$_LANG['total_amount_payable'] = '應付總額';
$_LANG['order_gift_integral'] = '訂單 %s 贈送的積分';

/* 缺貨處理 */
$_LANG['oos'][OOS_WAIT] = '等待所有商品備齊後再發';
$_LANG['oos'][OOS_CANCEL] = '取消訂單';
$_LANG['oos'][OOS_CONSULT] = '與店主協商';

/* 評論部分 */
$_LANG['username'] = '用戶名';
$_LANG['email'] = '電子郵件地址';
$_LANG['comment_rank'] = '評價等級';
$_LANG['comment_content'] = '評論內容';
$_LANG['submit_comment'] = '提交評論';
$_LANG['button_reset'] = '重置表單';
$_LANG['goods_comment'] = '商品評論';
$_LANG['article_comment'] = '文章評論';

/* 支付確認部分 */
$_LANG['pay_status'] = '支付狀態';
$_LANG['pay_not_exist'] = '此支付方式不存在或者參數錯誤！';
$_LANG['pay_disabled'] = '此支付方式還沒有被啟用！';
$_LANG['pay_success'] = '您此次的支付操作已成功！';
$_LANG['pay_fail'] = '支付操作失敗，請返回重試！';

/* 文章部分 */
$_LANG['new_article'] = '最新文章';
$_LANG['shop_notice'] = '商店公告';
$_LANG['order_already_received'] = "此訂單已經確認過了，感謝您在本站購物，歡迎再次光臨。";
$_LANG['order_invalid'] = '您提交的訂單不正確。';
$_LANG['act_ok'] = "謝謝您通知我們您已收到貨，感謝您在本站購物，歡迎再次光臨。";
$_LANG['receive'] = '收貨確認';
$_LANG['buyer'] = '買家';
$_LANG['next_article'] = '下一篇';
$_LANG['prev_article'] = '上一篇';

/* 虛擬商品 */
$_LANG['virtual_goods_ship_fail'] = '部分商品自動發貨失敗，請儘快聯繫商家重新發貨';

/* 選購中心 */
$_LANG['pick_out'] = '選購中心';
$_LANG['fit_count'] = "共有 %s 件商品符合條件";
$_LANG['goods_type'] = "商品類型";
$_LANG['remove_all'] = '移除所有';
$_LANG['advanced_search'] = '高級搜索';
$_LANG['activity'] = '本商品正在進行';
$_LANG['order_not_exists'] = "非常抱歉，沒有找到指定的訂單。請和網站管理員聯繫。";

$_LANG['promotion_time'] = '的時間為%s到%s，趕快來搶吧！';

/* 倒計時 */
$_LANG['goods_js']['day'] = '天';
$_LANG['goods_js']['hour'] = '小時';
$_LANG['goods_js']['minute'] = '分鐘';
$_LANG['goods_js']['second'] = '秒';
$_LANG['goods_js']['branch'] = '分';
$_LANG['goods_js']['time'] = '時';
$_LANG['goods_js']['end'] = '結束';

/*商品語言JS start*/
$_LANG['goods_js']['goods_attr_js'] = '請選擇商品規格類型';
/*商品語言JS end*/

$_LANG['favourable'] = '優惠活動';

/* 團購部分語言項 */
$_LANG['group_buy'] = '團購活動';
$_LANG['group_buy_goods'] = '團購商品';
$_LANG['gb_goods_name'] = '團購商品：';
$_LANG['gb_start_date'] = '開始時間：';
$_LANG['gb_end_date'] = '結束時間：';
$_LANG['gbs_pre_start'] = '該團購活動尚未開始，請繼續關注。';
$_LANG['gbs_under_way'] = '該團購活動正在火熱進行中，距離結束時間還有：';
$_LANG['gbs_finished'] = '該團購活動已結束，正在等待處理...';
$_LANG['gbs_succeed'] = '該團購活動已成功結束！';
$_LANG['gbs_fail'] = '該團購活動已結束，沒有成功。';
$_LANG['gb_price_ladder'] = '價格階梯：';
$_LANG['gb_ladder_amount'] = '數量';
$_LANG['gb_ladder_price'] = '價格';
$_LANG['gb_deposit'] = '保證金：';
$_LANG['gb_restrict_amount'] = '限購數量';
$_LANG['gb_limited'] = '限購';
$_LANG['gb_gift_integral'] = '贈送積分';
$_LANG['gb_cur_price'] = '當前價格：';
$_LANG['gb_valid_goods'] = '當前定購數量：';
$_LANG['gb_final_price'] = '成交價格：';
$_LANG['gb_final_amount'] = '成交數量：';
$_LANG['gb_notice_login'] = '提示：您需要先註冊成為本站會員並且登錄後，才能參加商品團購!';
$_LANG['gb_error_goods_lacking'] = '對不起，商品庫存不足，請您修改數量！';
$_LANG['gb_error_restrict_amount'] = '對不起，您購買的商品數量已達到限購數量！';
$_LANG['gb_error_status'] = '對不起，該團購活動已經結束或尚未開始，現在不能參加！';
$_LANG['gb_error_login'] = '對不起，您沒有登錄，不能參加團購，請您先登錄！';
$_LANG['group_goods_empty'] = '當前沒有團購活動';
$_LANG['go_back'] = '返回上一頁';

/* 預售部分語言項 */
$_LANG['presale_error_status'] = '對不起，該預售活動已經結束或尚未開始，現在不能參加！';
$_LANG['back_to_presale'] = '返回預售商品';

/* 拍賣部分語言項 */
$_LANG['auction'] = '拍賣活動';
$_LANG['auction_list'] = '拍賣活動列表';
$_LANG['act_status'] = '活動狀態';
$_LANG['au_start_price'] = '起拍價';
$_LANG['au_start_price_two'] = '起 拍 價';
$_LANG['au_end_price'] = '一口價';
$_LANG['au_end_price_two'] = '一 口 價';
$_LANG['au_amplitude'] = '加價幅度';
$_LANG['au_deposit'] = '保證金';
$_LANG['au_deposit_two'] = '保 證 金';
$_LANG['no_auction'] = '當前沒有拍賣活動';
$_LANG['au_pre_start'] = '該拍賣活動尚未開始';
$_LANG['au_under_way'] = '該拍賣活動正在進行中，距離結束時間還有：';
$_LANG['au_under_way_1'] = '該拍賣活動正在進行中';
$_LANG['au_bid_user_count'] = '已出價人數';
$_LANG['au_last_bid_price'] = '最後出價';
$_LANG['au_last_bid_user'] = '最後出價的買家';
$_LANG['au_last_bid_time'] = '最後出價時間';
$_LANG['au_finished'] = '該拍賣活動已結束';
$_LANG['au_bid_user'] = '買家';
$_LANG['au_bid_price'] = '出價';
$_LANG['au_bid_time'] = '時間';
$_LANG['au_bid_status'] = '狀態';
$_LANG['no_bid_log'] = '暫時沒有買家出價';
$_LANG['au_bid_ok'] = '領先';
$_LANG['out'] = '出局';
$_LANG['au_i_want_bid'] = '我要出價';
$_LANG['nin_want_bid'] = '您的出價';
$_LANG['button_bid'] = '出價';
$_LANG['button_buy'] = '立即購買';
$_LANG['au_not_under_way'] = '拍賣活動已結束，不能再出價了';
$_LANG['au_bid_price_error'] = '請輸入正確的價格';
$_LANG['button_bid_succeed'] = '成功出價';
$_LANG['au_bid_after_login'] = '您只有註冊成為會員並且登錄之後才能出價';
$_LANG['au_bid_repeat_user'] = '您已經是這個商品的最高出價人了!';
$_LANG['au_your_lowest_price'] = '您的出價不能低於 %s';
$_LANG['au_your_lowest_price_wen'] = '您的出價不能低於 ¥';
$_LANG['au_user_money_short'] = '您的可用資金不足，請先到用戶中心充值!';
$_LANG['au_unfreeze_deposit'] = '解凍拍賣活動的保證金：%s';
$_LANG['au_freeze_deposit'] = '凍結拍賣活動的保證金：%s';
$_LANG['au_not_finished'] = '該拍賣活動尚未結束，不能購買';
$_LANG['au_order_placed'] = '您已經下過訂單了，如果您想重新購買，請先取消原來的訂單';
$_LANG['au_no_bid'] = '該拍賣活動沒有人出價，不能購買';
$_LANG['au_final_bid_not_you'] = '您不是最高出價者，不能購買';
$_LANG['au_buy_after_login'] = '請您先登錄';
$_LANG['au_is_winner'] = '恭喜您！您已經贏得了該商品的購買權.您可以點擊購買按鈕將寶貝買回家。';
$_LANG['au_mechanism'] = '拍賣機構';
$_LANG['bidding_process'] = '競拍流程';
$_LANG['process_step_one'] = '用戶賬號當前餘額不低於保證金金額';
$_LANG['process_step_two'] = '保證競拍結束時出價最高，獲得競拍商品';
$_LANG['process_step_three'] = '填寫訂單信息，提交訂單';
$_LANG['process_step_four'] = '支付已提交的訂單';
$_LANG['process_step_five'] = '支付成功後等待收貨，競拍完成';
$_LANG['auction_desc'] = '競拍詳情';
$_LANG['au_introduce'] = '拍賣介紹';
$_LANG['au_raiders'] = '競拍攻略';
$_LANG['au_ren'] = '競拍人';
$_LANG['rec_au'] = '推薦拍品';
$_LANG['bid_number'] = '出價人次';
$_LANG['au_end'] = '競拍結束';
$_LANG['more'] = '更多';
$_LANG['login'] = '登錄';
$_LANG['register'] = '註冊';
$_LANG['please_login'] = '请登錄';

/* 批發部分語言項 */
$_LANG['ws_user_rank'] = '您的等級暫時無法查看批發方案';
$_LANG['ws_login_please'] = '請您先登錄';
$_LANG['ws_return_home'] = '返回首頁';
$_LANG['ws_return_wholesale'] = '返回批發市場';
$_LANG['wholesale'] = '批發';
$_LANG['no_wholesale'] = '沒有批發商品';
$_LANG['ws_price'] = '批發價';
$_LANG['ws_subtotal'] = '小計';
$_LANG['ws_invalid_goods_number'] = '請輸入正確的數量';
$_LANG['ws_attr_not_matching'] = '您選擇的商品屬性不存在，請參照批發價格單選擇';
$_LANG['ws_goods_number_not_enough'] = '您購買的數量沒有達到批發的最小數量，請參照批發價格單';
$_LANG['ws_goods_attr_exists'] = "該商品已經在購物車中，不能再次加入";
$_LANG['ws_remark'] = '請輸入您的聯繫方式、付款方式和配送方式等信息';
$_LANG['ws_order_submitted'] = '您的訂單已提交成功，請記住您的訂單號: %s。';
$_LANG['ws_price_list'] = '價格單';
$_LANG['give_integral'] = '商品贈送積分';

/* 積分兌換部分語言項 */
$_LANG['exchange'] = '積分商城';
$_LANG['exchange_integral'] = '消耗積分：';
$_LANG['exchange_goods'] = '立刻兌換';
$_LANG['eg_error_login'] = '對不起，您沒有登錄，不能參加兌換，請您先登錄！';
$_LANG['eg_error_status'] = '對不起，該商品已經取消，現在不能兌換！';
$_LANG['eg_error_integral'] = '對不起，您現有的積分值不夠兌換本商品！';
$_LANG['notice_eg_integral'] = '積分商城商品需要消耗積分：';
$_LANG['eg_error_number'] = '對不起，該商品庫存不足，現在不能兌換！';

/* 會員登錄註冊 */
$_LANG['member_name'] = '會員';
$_LANG['password'] = '密碼';
$_LANG['confirm_password'] = '確認密碼';
$_LANG['sign_up'] = '註冊新會員';
$_LANG['forgot_password'] = '您忘記密碼了嗎？';
$_LANG['hello'] = '您好';
$_LANG['welcome_return'] = '歡迎您回來';
$_LANG['now_account'] = '您的賬戶中現在有';
$_LANG['balance'] = '餘額';
$_LANG['along_with'] = '以及';
$_LANG['preferential'] = '優惠券';
$_LANG['edit_user_info'] = '進入用戶中心';
$_LANG['logout'] = '退出';
$_LANG['user_logout'] = '退出';
$_LANG['welcome'] = "歡迎光臨本店";
$_LANG['user_login'] = '會員登陸';
$_LANG['login_now'] = '立即登陸';
$_LANG['reg_now'] = '立即註冊';

/* 商品品牌頁 */
$_LANG['official_site'] = '官方網站：';
$_LANG['brand_category'] = '分類瀏覽：';
$_LANG['all_category'] = '所有分類';
$_LANG['brand_desc'] = '品牌描述';
$_LANG['notime_desc'] = '暫無描述';
$_LANG['add_brand_info'] = '添加品牌信息';

/* 商品分類頁 */
$_LANG['goods_filter'] = '商品篩選';

/* cls_image類的語言項 */
$_LANG['directory_readonly'] = '目錄 % 不存在或不可寫';
$_LANG['invalid_upload_image_type'] = '不是允許的圖片格式';
$_LANG['upload_failure'] = '文件 %s 上傳失敗。';
$_LANG['missing_gd'] = '沒有安裝GD庫';
$_LANG['missing_orgin_image'] = '找不到原始圖片 %s ';
$_LANG['nonsupport_type'] = '不支持該圖像格式 %s ';
$_LANG['creating_failure'] = '創建圖片失敗';
$_LANG['writting_failure'] = '圖片寫入失敗';
$_LANG['empty_watermark'] = '水印文件參數不能為空';
$_LANG['missing_watermark'] = '找不到水印文件%s';
$_LANG['create_watermark_res'] = '創建水印圖片資源失敗。水印圖片類型為%s';
$_LANG['create_origin_image_res'] = '創建原始圖片資源失敗，原始圖片類型%s';
$_LANG['invalid_image_type'] = '無法識別水印圖片 %s ';
$_LANG['file_unavailable'] = '文件 %s 不存在或不可讀';

/* 郵件發送錯誤信息 */
$_LANG['smtp_setting_error'] = '郵件伺服器設置信息不完整';
$_LANG['smtp_connect_failure'] = '無法連接到郵件伺服器 %s';
$_LANG['smtp_login_failure'] = '郵件伺服器驗證帳號或密碼不正確';
$_LANG['smtp_refuse'] = '伺服器拒絕發送該郵件';
$_LANG['sendemail_false'] = "郵件發送失敗，請與網站管理員聯繫！";
$_LANG['disabled_fsockopen'] = 'fsockopen函數被禁用';

$_LANG['topic_goods_empty'] = '當前沒有專題商品';
$_LANG['email_list_ok'] = '訂閱';
$_LANG['email_list_cancel'] = '退訂';
$_LANG['email_invalid'] = '郵件地址非法！';
$_LANG['email_alreadyin_list'] = '郵件地址已經存在於列表中！';
$_LANG['email_notin_list'] = '郵件地址不在列表中！';
$_LANG['email_re_check'] = '已經重新發送驗證郵件，請查收並確認！';
$_LANG['email_check'] = '請查收郵件進行確認操作！';
$_LANG['email_not_alive'] = '此郵件地址是未驗證狀態，不需要退訂！';
$_LANG['check_mail'] = '驗證郵件';
$_LANG['check_mail_content'] = "%s 您好：<br><br>這是由%s發送的郵件訂閱驗證郵件,點擊以下的鏈接地址,完成驗證操作。<br><a href=\"%s\" target=\"_blank\">%s</a>\n<br><br>%s<br>%s";
$_LANG['email_checked'] = '郵件已經被確認！';
$_LANG['hash_wrong'] = '驗證串錯誤！請核對驗證串或輸入email地址重新發送驗證串！';
$_LANG['email_canceled'] = '郵件已經被退定！';
$_LANG['goods_click_count'] = '商品點擊數';
$_LANG['p_y']['link_start'] = '<a href="http://www.ecmoban.com" target="_blank" style=" font-family:Verdana; font-size:11px;">';
$_LANG['p_y']['link_p'] = 'Powe';
$_LANG['p_y']['link_r'] = 'red&nbsp;';
$_LANG['p_y']['link_b'] = 'by&nbsp;';
$_LANG['p_y']['main_start'] = '<strong><span style="color: #3366FF">';
$_LANG['p_y']['main_e'] = 'E';
$_LANG['p_y']['main_c'] = 'CSho';
$_LANG['p_y']['main_p'] = 'p</span>&nbsp;';
$_LANG['p_y']['v_s'] = '<span style="color: #FF9966">';
$_LANG['p_y']['v'] = VERSION;
$_LANG['p_y']['link_end'] = '</span></strong></a>&nbsp;';

/* 虛擬卡 */
$_LANG['card_sn'] = '卡片序號';
$_LANG['card_password'] = '卡片密碼';
$_LANG['end_date'] = '截至日期';
$_LANG['virtual_card_oos'] = '虛擬卡已缺貨';

/* 訂單狀態查詢 */
$_LANG['invalid_order_sn'] = '無效訂單號';
$_LANG['order_status'] = '訂單狀態';
$_LANG['shipping_date'] = '發貨時間';
$_LANG['query_order'] = '查詢該訂單號';
$_LANG['order_query_toofast'] = '您的提交頻率太高，歇會兒再查吧。';

$_LANG['online_info'] = '當前共有 %s 人在線';

/* 按鈕 */
$_LANG['btn_direct_buy'] = '直接購買';
$_LANG['btn_buy'] = '購買';
$_LANG['btn_collect'] = '收藏';
$_LANG['btn_store_pick'] = '到店取貨';
$_LANG['btn_add_to_cart'] = "加入購物車";
$_LANG['btn_add_to_collect'] = '添加收藏';
$_LANG['cancel_collect'] = '取消收藏';

$_LANG['stock_up'] = '缺貨';

$_LANG['hot_search'] = '熱門搜索';

$_LANG['please_select_attr'] = "你加入購物車的商品有不同型號可選，你是否要立即跳轉到商品詳情選擇型號？";

/* 促銷信息欄 */
$_LANG['snatch_promotion'] = '[奪寶]';
$_LANG['group_promotion'] = '[團購]';
$_LANG['auction_promotion'] = '[拍賣]';
$_LANG['favourable_promotion'] = '[優惠]';
$_LANG['wholesale_promotion'] = '[批發]';
$_LANG['package_promotion'] = '[禮包]';

/* feed推送 */
$_LANG['feed_user_buy'] = "購買了";
$_LANG['feed_user_comment'] = "評論了";
$_LANG['feed_goods_price'] = "商品價格";
$_LANG['feed_goods_desc'] = "商品描述";

/* 留言板 */
$_LANG['shopman_comment'] = '商品評論';
$_LANG['message_ping'] = '評';
$_LANG['message_board'] = "留言板";
$_LANG['post_message'] = "我要留言";
$_LANG['message_title'] = '主題';
$_LANG['message_time'] = '留言時間';
$_LANG['reply_time'] = '回復時間';
$_LANG['reply_browse'] = '回復/瀏覽';
$_LANG['shop_owner_reply'] = '店主回復';
$_LANG['message_board_type'] = '留言類型';
$_LANG['content'] = '內容';
$_LANG['message_content'] = '留言內容';
$_LANG['message_anonymous'] = '匿名留言';
$_LANG['message_type'][M_MESSAGE] = '留言';
$_LANG['message_type'][M_COMPLAINT] = '投訴';
$_LANG['message_type'][M_ENQUIRY] = '詢問';
$_LANG['message_type'][M_CUSTOME] = '售後';
$_LANG['message_type'][M_BUY] = '求購';
$_LANG['message_type'][M_BUSINESS] = '商家留言';
$_LANG['message_type'][M_COMMENT] = '評論';
$_LANG['message_board_js']['msg_empty_email'] = '請輸入您的電子郵件地址';
$_LANG['message_board_js']['msg_error_email'] = '電子郵件地址格式不正確';
$_LANG['message_board_js']['msg_title_empty'] = '留言標題為空';
$_LANG['message_board_js']['msg_content_empty'] = '留言內容為空';
$_LANG['message_board_js']['msg_captcha_empty'] = '驗證碼為空';
$_LANG['message_board_js']['msg_title_limit'] = '留言標題不能超過200個字';
$_LANG['message_submit_wait'] = '您的留言已成功發表, 請等待管理員的審核!';
$_LANG['message_submit_done'] = '發表留言成功';
$_LANG['message_board_close'] = "暫停留言板功能";
$_LANG['upload_file_limit'] = '文件大小超過了限制 %dKB';
$_LANG['message_list_lnk'] = '返回留言列表';

/* 報價單 */
$_LANG['quotation'] = "報價單";
$_LANG['print_quotation'] = "列印報價單";
$_LANG['goods_inventory'] = "庫存";
$_LANG['goods_category'] = "商品分類";
$_LANG['shopman_reply'] = '管理員回復';
$_LANG['specifications'] = '規格';

/* 相冊JS語言項 */
$_LANG['gallery_js']['close_window'] = '您是否關閉當前窗口';


$_LANG['submit'] = '提 交';
$_LANG['reset'] = '重 置';
$_LANG['order_query'] = '訂單查詢';
$_LANG['shipping_query'] = '發貨查詢';
$_LANG['view_history'] = '瀏覽歷史';
$_LANG['clear_history'] = '[清空]';
$_LANG['no_history'] = '您已清空最近瀏覽過的商品';
$_LANG['goods_tag'] = '商品標籤';
$_LANG['releate_goods'] = "用戶還喜歡";
$_LANG['goods_list'] = '商品列表';
$_LANG['favourable_goods'] = '收藏該商品';
$_LANG['accessories_releate'] = '相關配件';
$_LANG['article_releate'] = '相關文章';
$_LANG['email_subscribe'] = '郵件訂閱';
$_LANG['consignee_info'] = '收貨人信息';
$_LANG['user_comment'] = '用戶評論';
$_LANG['total'] = '共';
$_LANG['user_comment_num'] = '條評論';
$_LANG['auction_goods'] = '拍賣商品';
$_LANG['auction_goods_info'] = '拍賣商品詳情';
$_LANG['article_cat'] = '文章分類';
$_LANG['online_vote'] = '在線調查';
$_LANG['new_price'] = '最新出價';
$_LANG['promotion_info'] = '促銷信息';
$_LANG['price_grade'] = '價格範圍';
$_LANG['your_choice'] = '您的選擇';
$_LANG['system_info'] = '系統信息';
$_LANG['system_error'] = '系統出錯，請重試';
$_LANG['all_tags'] = '所有標籤';
$_LANG['activity_list'] = '活動列表';
$_LANG['package_list'] = '禮包列表';
$_LANG['treasure_info'] = '寶貝詳情';
$_LANG['activity_desc'] = '活動描述';
$_LANG['activity_intro'] = '活動介紹';
$_LANG['get_password'] = '找回密碼';
$_LANG['fee_total'] = '費用總計';
$_LANG['other_info'] = '其它信息';
$_LANG['user_balance'] = '會員餘額';
$_LANG['wholesale_goods_cart'] = "批發商品購物車";
$_LANG['wholesale_goods_list'] = '批發商品列表';
$_LANG['bid_record'] = '出價記錄';
$_LANG['shipping_method'] = '配送方式';
$_LANG['shipping_number'] = '配送單號';
$_LANG['payment_method'] = '支付方式';
$_LANG['wxpay_pay'] = "微信支付";
$_LANG['goods_package'] = '商品包裝';
$_LANG['goods_card'] = '祝福賀卡';
$_LANG['groupbuy_intro'] = '團購說明';
$_LANG['groupbuy_goods_info'] = '團購商品詳情';
$_LANG['act_time'] = '起止時間';
$_LANG['toprank'] = '排行榜';
$_LANG['top10'] = '銷售排行';
$_LANG['service_guarantee'] = '服務保障';
$_LANG['Commodity_number'] = '商品編號';
$_LANG['snapshot_time'] = "快照時間";
$_LANG['attr'] = "屬性";
$_LANG['specification'] = "規格參數";
$_LANG['description'] = '商品詳情';
$_LANG['view_image'] = '查看圖片';
$_LANG['online_payment'] = '在線支付';
$_LANG['remarks'] = '備註';
$_LANG['latest_price'] = '最新價';

/* 優惠活動 */
$_LANG['label_act_name'] = '優惠活動名稱：';
$_LANG['label_start_time'] = '優惠開始時間：';
$_LANG['label_end_time'] = '優惠結束時間：';
$_LANG['label_user_rank'] = '享受優惠的會員等級：';
$_LANG['not_user'] = '非會員';
$_LANG['label_act_range'] = '優惠範圍：';
$_LANG['far_all'] = '全部商品';
$_LANG['far_category'] = '以下分類';
$_LANG['far_brand'] = '以下品牌';
$_LANG['far_goods'] = '以下商品';
$_LANG['label_min_amount'] = '金額下限：';
$_LANG['label_max_amount'] = '金額上限：';
$_LANG['notice_max_amount'] = '0表示沒有上限';
$_LANG['label_act_type'] = '優惠方式：';
$_LANG['fat_goods'] = '享受贈品';
$_LANG['fat_price'] = '現金減免';
$_LANG['fat_discount'] = '享受折扣';
$_LANG['orgtotal'] = '原始價格';
$_LANG['heart_buy'] = '心動不如行動';
$_LANG['activity_txt'] = '活動';
$_LANG['activity_time'] = '活動時間';

/* 其他模板涉及常用語言項 */
$_LANG['label_regist'] = '用戶註冊';
$_LANG['label_login'] = '用戶登錄';
$_LANG['label_profile'] = '用戶信息';
$_LANG['label_collection'] = '我的收藏';
$_LANG['article_list'] = '文章列表';
$_LANG['preferences_price'] = '單品價格';
$_LANG['divided_into'] = '分成規則';

//店鋪街by wang
$_LANG['store_street'] = '店鋪街';
$_LANG['brand_street'] = '品牌街';
$_LANG['mini_sns'] = '微社區';

$_LANG['no_goods_in_cart'] = "您的購物車中沒有商品！";
$_LANG['no_consignee'] = "請填寫您的收貨地址";
$_LANG['over_bind_limit'] = "您即將綁定的儲值卡已超出該類卡可綁定上限！";

//瀑布流載入分類商品 by wu
$_LANG['add_to_cart'] = '加入購物車';
$_LANG['sales_volume'] = '銷量';
$_LANG['compare'] = '對比';
$_LANG['collect'] = '收藏';
$_LANG['have_no_goods'] = '暫時缺貨';
$_LANG['select_attr'] = '請選擇屬性';
$_LANG['order_wholesale_sms'] = '訂單批發信息';

/* 眾籌title */
$_LANG['crowdfunding'] = '眾籌項目';
$_LANG['zc_search'] = '搜索眾籌項目';
$_LANG['zc_order_info'] = '訂單信息';
$_LANG['zc_order_submit'] = '訂單提交';

/*門店 by kong 20160722*/
$_LANG['store_shortage'] = "對不起，該商品門店庫存不足，請選擇其他門店";


/*頁面底部  by kong haojlj*/
$_LANG['Authentic_guarantee'] = "正品保障";
$_LANG['Rave_reviews'] = "好評如潮";
$_LANG['7_days_return'] = '七天包退';
$_LANG['Lightning_delivery'] = '閃電發貨';
$_LANG['Authority_of_honor'] = '權威榮譽';
$_LANG['Coupon_redemption_succeed'] = "領取成功！感謝您的參與，祝您購物愉快~";
$_LANG['coupons_prompt'] = "本活動為概率性事件，不能保證所有客戶成功領取優惠券";
$_LANG['Site_navigation'] = "網站導航";
$_LANG['seller_store'] = '店鋪';
$_LANG['all_goods_cat'] = "全部商品分類";
$_LANG['Good_coupon_market'] = "好券集市";
$_LANG['task_mart'] = "任務集市";
$_LANG['presell_cat'] = "全部預售分類";
$_LANG['New_product_release'] = "新品發布";
$_LANG['First_order'] = "搶先定";
$_LANG['piece_total'] = "件共計";
$_LANG['go_to_cart'] = "去購物車";
$_LANG['Welcome_to'] = "歡迎來到";
$_LANG['please_login'] = "請登錄";
$_LANG['registered'] = "登錄";
$_LANG['download_prompt'] = "下載文件不存在！";
$_LANG['flagship_store'] = '旗艦店';
$_LANG['exclusive_shop'] = '專賣店';
$_LANG['franchised_store'] = '專營店';
$_LANG['week'][1] = '星期一';
$_LANG['week'][2] = '星期二';
$_LANG['week'][3] = '星期三';
$_LANG['week'][4] = '星期四';
$_LANG['week'][5] = '星期五';
$_LANG['week'][6] = '星期六';
$_LANG['week'][7] = '星期日';
$_LANG['table_prompt'] = "表名稱或表欄位名稱不能為空";
$_LANG['presell'] = "預售";
$_LANG['sold_alt'] = "已售";
$_LANG['gift_card_exchange'] = "禮品卡兌換";
$_LANG['project'] = "專題";
$_LANG['rec_txt'][1] = '普通';
$_LANG['rec_txt'][2] = '團購';
$_LANG['rec_txt'][3] = '拍賣';
$_LANG['rec_txt'][4] = '奪寶奇兵';
$_LANG['rec_txt'][5] = '積分商城';
$_LANG['rec_txt'][6] = '預售';
$_LANG['rec_txt'][7] = '專題';
$_LANG['rec_txt'][8] = '眾籌';
$_LANG['rec_txt'][9] = '促銷活動';
$_LANG['rec_txt'][10] = '秒殺';
$_LANG['rec_txt'][11] = '超值禮包';
$_LANG['rec_txt'][12] = '拼團';
$_LANG['rec_txt'][13] = '砍價';
$_LANG['cat_list'] = '購物車';
$_LANG['my_cart'] = '我的購物車';
$_LANG['back'] = '返回';
$_LANG['preservation'] = '保存';
$_LANG['Since_some'] = "市自提點";
$_LANG['implement_time'] = "執行時間：";
$_LANG['information_null'] = "抱歉，沒有找到符合條件的數據";
$_LANG['Shopping_together_single'] = "購物湊單";
$_LANG['Contact_us'] = "聯繫我們";
$_LANG['submit_goods'] = '提交';
$_LANG['submit_confirm'] = '確認提交';
$_LANG['reset_alt'] = '重置';
$_LANG['Mall_announcement'] = "商城公告";
$_LANG['System_cat'] = "系統分類";
$_LANG['Other_information'] = "其他信息";
$_LANG['Recent_popular'] = "近期熱門";
$_LANG['View_larger'] = "查看大圖";
$_LANG['general_audience'] = '全場通用';
$_LANG['Send_light'] = '送照明燈';
$_LANG['Jingdong_peisong'] = "京東配送";
$_LANG['Please_select'] = "請選擇..";

/*20161214  start*/
$_LANG['View_details'] = "查看詳情";
$_LANG['Browsing_record'] = "瀏覽記錄";
$_LANG['default'] = "默認";
$_LANG['is_new'] = '新品';
$_LANG['is_hot'] = '熱賣';
$_LANG['is_best'] = '精品';
$_LANG['Comment_number'] = "評論數";
$_LANG['clear'] = "清空";
$_LANG['Receiving_land'] = "收貨地";
$_LANG['Free_shipping'] = "包郵";
$_LANG['Self_goods'] = "自營商品";
$_LANG['Grid_model'] = "網格模式";
$_LANG['List_model'] = "列表模式";
$_LANG['Recent_browse'] = "最近瀏覽";
$_LANG['tuiguang_goods'] = "推廣商品";
$_LANG['Popular_recommendation'] = "熱門推薦";
$_LANG['torob_buy'] = "去搶購";
$_LANG['brand_goods'] = "品牌商品";
$_LANG['Enter_brand_page'] = "進入品牌頁";
$_LANG['Prompt'] = "提示";
$_LANG['Cancel_attention'] = "取消關注";
$_LANG['check_all'] = '全選';
$_LANG['check_all_back'] = '全選/反選';
$_LANG['Share'] = "共有";
$_LANG['pic_count'] = "張圖片";
$_LANG['cilck_view'] = "點擊查看";
$_LANG['cilck_retract'] = "點擊收起";
$_LANG['View_all_replies'] = "查看全部回復";
$_LANG['Collection_success'] = "您已成功收藏該商品！";
$_LANG['Collection_see'] = "您已成功收藏該商品！";
$_LANG['remove_goods_Collection'] = "您確定要將該商品從收藏夾中刪除嗎？";
$_LANG['cancel_grand'] = "您確定要取消關注該品牌嗎？";
$_LANG['add_address_zc'] = "添加收貨地址";
$_LANG['confirm_address_zc'] = "確定收貨地址";
$_LANG['Basic_info_comp'] = "基本信息對比";
$_LANG['Model'] = "型號";
$_LANG['contrast_item'] = "暫無對比項";
$_LANG['compare_click_see'] = "點擊這裡摺疊/展開";
$_LANG['compare_one'] = "點擊取消固定";
$_LANG['compare_two'] = "取消高亮顯示不同項";
$_LANG['compare_three'] = "高亮顯示不同項";
$_LANG['compare_four'] = "顯示相同項";
$_LANG['compare_five'] = "隱藏相同項";
$_LANG['add_consignee_address'] = '新增收貨人地址';
$_LANG['edit_consignee_info'] = '編輯收貨人信息';
$_LANG['remove_consignee_info'] = '刪除收貨人信息';
$_LANG['Consignee'] = "收貨人";
$_LANG['input_Consignee_name'] = "請您填寫收貨人姓名";
$_LANG['Local_area'] = '所在地區';
$_LANG['address_info'] = "詳細信息";
$_LANG['input_address_info'] = "請您填寫收貨詳細地址";
$_LANG['phone_con'] = "手機號碼";
$_LANG['telephone'] = "電話";
$_LANG['Fixed_telephone'] = "固定電話";
$_LANG['not_set_telephone'] = "未設置電話";
$_LANG['input_contact'] = "請您填寫收貨人聯繫方式";
$_LANG['con_email'] = "郵箱";
$_LANG['con_email_input'] = "請您填寫郵箱";
$_LANG['con_sign_building'] = '地址別名';
$_LANG['inputcon_sign_building'] = '請您填寫地址別名';
$_LANG['sign_building_desc'] = "設置一個易記的名稱，如：'送到家裡'、'送到公司'";
$_LANG['Zip_code'] = "郵編";
$_LANG['deliver_goods_time'] = '最佳送貨時間';
$_LANG['con_Preservation'] = '保存收貨人信息';
$_LANG['goods_info'] = "商品信息";
$_LANG['shop_Price_dis'] = "商城價";
$_LANG['Evaluation_points'] = "評價分";
$_LANG['tiao_dis'] = "條";
$_LANG['Hot_topic'] = "熱門話題";
$_LANG['hidden'] = '隱藏';
$_LANG['Contrast_bar'] = "對比欄";
$_LANG['Continue_add_dui'] = "您還可以繼續添加";
$_LANG['empty_contrast'] = '清空對比欄';
$_LANG['flow_info_lbi'] = "總價(不含運費)";
$_LANG['Selling_price'] = "售　　價";
$_LANG['selling_price_alt'] = "售價";
$_LANG['minimum'] = "起購量";
$_LANG['gs_number'] = '數　　量';
$_LANG['Rate'] = '好評';
$_LANG['zhong_p'] = '中評';
$_LANG['Bad'] = '差評';
$_LANG['share_flow'] = "分享";
$_LANG['report'] = "舉報";
$_LANG['make_appointment'] = "預約";
$_LANG['rob_make_appointment'] = "搶先預購";
$_LANG['existing'] = "已有";
$_LANG['No_goods'] = "無貨";
$_LANG['Customer_service_p'] = "客服";
$_LANG['freight_p'] = '運費';
$_LANG['jian'] = "件";
$_LANG['yuan'] = "元";
$_LANG['ge'] = "個";
$_LANG['zhang'] = "張";
$_LANG['seller'] = '商家';
$_LANG['seller_Grade'] = '商家等級';
$_LANG['brand_gm'] = '品&nbsp;&nbsp;&nbsp;&nbsp;牌';
$_LANG['Go_to_store'] = "進店逛逛";
$_LANG['go_see'] = "去看看";
$_LANG['Deliver_to'] = "送貨至";
$_LANG['Sold'] = "售出";
$_LANG['his_bi'] = "件";
$_LANG['mail'] = "郵";
$_LANG['retreat'] = "退";
$_LANG['go_shoping'] = "去購物";
$_LANG['Flash_sale'] = "限時搶購";
$_LANG['hour_two'] = '時';
$_LANG['minutes'] = '分';
$_LANG['seconds'] = '秒';
$_LANG['Rush_once'] = "立即搶";
$_LANG['rush_renshu'] = "人已搶購";
$_LANG['Brand_Street'] = "品牌街";
$_LANG['Mall_info'] = "商城資訊";
$_LANG['people_participate'] = "人已參加";
$_LANG['zhe'] = "折";
$_LANG['shopping'] = "購";
$_LANG['say'] = "說";
$_LANG['Group_purchase_end'] = "團購結束";
$_LANG['Group_purchase_now'] = "立即團";
$_LANG['End_pitch'] = "距結束";
$_LANG['Wait_shooting'] = "等待開拍";
$_LANG['Exchange_price'] = "換購價";
$_LANG['combo_markPrice'] = "參考價";
$_LANG['integral'] = '積分';
$_LANG['integral_coune'] = '人兌換';
$_LANG['exchange_now'] = '立即兌換';
$_LANG['Detailed_map'] = "詳細地圖";
$_LANG['Track'] = "跟蹤";
$_LANG['all_order'] = "全部訂單";
$_LANG['View_all_orders'] = "查看全部訂單";
$_LANG['Distribution_limit'] = "商品暫時只支持配送至中國大陸地區";
$_LANG['payment_is_cod'] = '貨到付款';
$_LANG['Classification_selection'] = "分類篩選";
$_LANG['successfully_added_shoping'] = "寶貝已成功添加到購物車！";
$_LANG['cart_count'] = "購物車共有";
$_LANG['Baby_total_amount'] = "寶貝總金額為";
$_LANG['cart_baby_success'] = "已成功添加到購物袋！";
$_LANG['zhong_boby'] = "種寶貝";
$_LANG['total_cart'] = '合計';
$_LANG['pay_to_cart'] = "去購物袋結算";
$_LANG['Detailed_score'] = '評分詳細';
$_LANG['my_take_delivery'] = "我的提貨";
$_LANG['level_pos'] = "級&nbsp;別";
$_LANG['temporary_no'] = "暫無";
$_LANG['order_list'] = '我的訂單';
$_LANG['My_assets'] = "我的資產";
$_LANG['My_footprint'] = '我的足跡';
$_LANG['Email_subscription'] = "郵箱訂閱";
$_LANG['Customer_service_center'] = "客服中心";
$_LANG['email_posi'] = "請輸入您的郵箱帳號";
$_LANG['Support_project'] = "支持項目";
$_LANG['Launch'] = "發起";
$_LANG['Support'] = '支持';
$_LANG['zc_see_content'] = "點擊顯示更多回復內容";
$_LANG['input_number_desc'] = "還可以輸入";
$_LANG['zi_zc'] = "字";
$_LANG['zhuce'] = "免費註冊";
$_LANG['zhi'] = "至";
$_LANG['distribution'] = "配送";
$_LANG['comment_sunburn'] = "評論曬單";
$_LANG['serch_goods'] = '搜商品';
$_LANG['serch_shop'] = '搜店鋪';
$_LANG['serch_whole_station'] = '搜全站';
$_LANG['serch_our_shop'] = '搜本店';
$_LANG['serch_want_buy'] = '發布求購單';

$_LANG['switch'] = "切換";
$_LANG['select_country'] = "選擇國別";
$_LANG['different_countries'] = "配&nbsp;&nbsp;送";
$_LANG['click_countries'] = "點擊切換國別";

$_LANG['by_stages'] = "分期";
$_LANG['Sun_to_single'] = "去曬單";
$_LANG['goods_null_cart'] = "購物車中還沒有商品，趕緊選購吧！";
$_LANG['sort'] = '排序';
$_LANG['value_card'] = "儲值卡";
$_LANG['value_card_bind'] = "儲值卡綁定";
$_LANG['in_value_card_bind'] = "已綁定儲值卡";
$_LANG['value_card_instructions'] = "<h3>儲值卡綁定與說明</h3>
                <p>1、在上側輸入儲值卡卡號密碼進行綁定</p>
                <p>2、請仔細查閱儲值卡使用範圍</p>";
$_LANG['wu'] = "無";
$_LANG['checked_city'] = "切換城市";
$_LANG['hot_city'] = "熱門城市";
$_LANG['sufficient'] = "充足";
$_LANG['only_leave'] = "僅剩";
$_LANG['time_shop'] = "到店時間";
$_LANG['take_time_desc'] = "默認為一天後的當前時間哦";
$_LANG['store_take_mobile'] = "請輸入手機號碼";
$_LANG['store_take_mobile_one'] = "手機號碼將獲取取貨碼訂單哦";
$_LANG['address'] = '地址';
$_LANG['sales_hotline'] = "銷售熱線";
$_LANG['working_time'] = "工作時間";
$_LANG['change_choice'] = "更改選擇";
$_LANG['change_choice_desc'] = "該地區暫無門店或屬性沒有庫存";
$_LANG['select_store_info'] = "選擇門店";
$_LANG['select_store_info_notic'] = "請選擇有貨的門店，上門自提";
$_LANG['wu'] = "無";
$_LANG['all_goods'] = '全部商品';
$_LANG['spec_cat'] = '指定分類';
$_LANG['spec_goods'] = '指定商品';
$_LANG['all_goods_explain'] = '限平台自營商品使用';
$_LANG['spec_cat_explain'] = '限平台指定分類使用：% ';
$_LANG['spec_goods_explain'] = '限平台指定商品使用：<a href="javascript:;" onclick="specGoods()" style="color:red;" >點此查看</a>';

$_LANG['label_mobile'] = '手機號碼';
$_LANG['get_verification_code_user'] = "獲取簡訊校驗碼";
$_LANG['Un_bind'] = "解除綁定";
$_LANG['Immediately_verify'] = "立即驗證";
$_LANG['package_nonumer'] = "對不起，該超值禮包已經庫存不足。請選擇其他禮包或者聯繫管理員！";
$_LANG['big_pic'] = "大圖";
$_LANG['Small_pic'] = "小圖";
$_LANG['pattern'] = "模式";
$_LANG['peer_comparison'] = "同行相比";
$_LANG['recommended_store'] = "推薦店鋪";

$_LANG['update_Success'] = "更新成功！";
$_LANG['Submit_Success'] = "提交成功！";
$_LANG['delete_success'] = "刪除成功";
$_LANG['Submit_fail'] = "提交失敗，稍後在提交一次";

$_LANG['indate'] = '有效期';
$_LANG['bonus_card_number'] = "卡&nbsp;&nbsp;&nbsp;號";
$_LANG['keyong'] = "可用";
$_LANG['range_bonus'] = "范   圍";
$_LANG['settled_down'] = "我要入駐";
$_LANG['settled_down_schedule_step'] = "進度查詢";
$_LANG['Only_have_inventory'] = "僅顯示有貨";
$_LANG['staircase_price'] = "階梯價";
$_LANG['view_staircase_price'] = "查看階梯價格";

$_LANG['captcha_notic'] = "看不清，點擊圖片換一張";

$_LANG['isHas_warehouse_num'] = "<strong>有貨</strong>，下單後立即發貨";

/* 審核 */
$_LANG['adopt_status'] = '審核狀態';
$_LANG['have_audited'] = '已審核';
$_LANG['not_audited'] = '未審核';
$_LANG['not_through'] = '未通過';
$_LANG['yes_through'] = '通過';
$_LANG['audited_adopt'] = '審核通過';
$_LANG['audited_yes_adopt'] = '審核已通過';
$_LANG['audited_not_adopt'] = '審核未通過';
$_LANG['adopt_reply'] = '審核回復';
$_LANG['wuxu_adopt'] = '無需審核';
$_LANG['adopt_goods'] = '審核商品';
$_LANG['adopt_brand'] = '審核品牌';
$_LANG['adopt_edit'] = '審核修改';

/* 秒殺活動 */
$_LANG['merchant_rec'] = "店鋪推薦";
$_LANG['self_run'] = "自營";
$_LANG['platform_self'] = '平台自營';
$_LANG['all_merchants'] = '所有店鋪';
$_LANG['self_merchants'] = '自營店鋪';
$_LANG['assign_merchants'] = '指定店鋪';
$_LANG['overdue_login'] = '登陸過期，請重新登陸！';
$_LANG['no_address'] = '收貨地址不能為空!';
$_LANG['no_priv_address'] = '您沒有許可權操作他人收貨地址!';
$_LANG['cat_not_null'] = "該分類下沒有內容";

$_LANG['ju_start'] = "距開始";
$_LANG['ju_end'] = "距結束";
$_LANG['more_secKill'] = "更多秒殺";


//b2b
$_LANG['wholesale_more'] = '更多批發';
$_LANG['immediate_purchase'] = '立即採購';
$_LANG['purchase_time'] = '限時採購';
$_LANG['adopt_edit'] = '立即採購';

$_LANG['not_seller_user'] = '您不是商家用戶，無權查看此頁面!';
$_LANG['not_login_user'] = '您還沒有登錄，無權查看此頁面!';

/* lbi文件 */
$_LANG['browse'] = '瀏覽';
$_LANG['not_discussion'] = '暫無討論帖';
$_LANG['click_browse_all'] = '點擊瀏覽全部';
$_LANG['not_evaluate'] = '暫無評價';
$_LANG['not_conversation'] = '暫無談話';
$_LANG['not_data'] = '暫無數據';
$_LANG['add_new_address'] = '添加新地址';
$_LANG['darren_area'] = '達人專區';
$_LANG['go_to_see'] = '去見識';
$_LANG['not_set_attrtype_contrast'] = '沒有設置屬性類型無法對比';
$_LANG['not_see_enough'] = '還沒逛夠';
$_LANG['recommended_store_notic'] = '時尚新活力時尚新';

//討論圈lbi
$_LANG['yu'] = "預";
$_LANG['cumulative_comment'] = "累計評論";
$_LANG['my_dispost_post'] = "我要發帖";
$_LANG['hot_dispost_post'] = "熱門帖子";

// 發票lbi
$_LANG['need_invoice'][0] = "普通發票";
$_LANG['need_invoice'][1] = "增值稅專用發票";

$_LANG['invoice_ordinary'] = "普通發票（紙質）";

$_LANG['iis_state_one'] = '您的增票資質已提交，等待審核。';
$_LANG['iis_state_two'] = '恭喜！您的增票資質信息審核通過。';
$_LANG['iis_state_three'] = '點擊查看增票信息';
$_LANG['iis_state_four'] = '抱歉！您的增票資質信息錯誤，審核不通過。';

$_LANG['iis_state_tab_one'] = '選擇開票方式';
$_LANG['iis_state_tab_two'] = '填寫或核對公司信息';
$_LANG['iis_state_tab_three'] = '填寫收票人信息';
$_LANG['iis_state_tab_notic'] = '發票將在訂單完成之後7-15個工作日寄出';

$_LANG['invoice_mode'] = '開票方式';
$_LANG['order_complete_invoice'] = '訂單完成後開票';
$_LANG['detailed'] = '明細';

$_LANG['label_company_name'] = '單位名稱';
$_LANG['label_tax_id'] = '納稅人識別碼';
$_LANG['label_vat_name'] = '收票人姓名';
$_LANG['label_vat_phone'] = '收票人手機號';
$_LANG['label_bank_account'] = '銀行賬號';
$_LANG['label_vat_address'] = '收票人詳細地址';
$_LANG['label_company_address'] = '註冊地址';
$_LANG['label_company_telephone'] = '註冊電話';
$_LANG['label_bank_of_deposit'] = '開戶銀行';

//flow lbi
$_LANG['address_prompt_two'] = '請填寫收貨地址';
$_LANG['not_set_shipping'] = "未設置配送方式";
$_LANG['not_set_address'] = "收貨人信息不完整";
$_LANG['cross_order_excess'] = '店鋪商品超出';

$_LANG['new_arrivals'] = '新品推薦';
$_LANG['new_first_start'] = '新品首發';
$_LANG['sett_title'] = '馬上入駐 開向未來';
$_LANG['mobile_mall'] = '手機商城';
$_LANG['bid_ci_number'] = '出價次數';
$_LANG['rz_shopName'] = "送拍機構";
$_LANG['cart_goods_number'] = "件商品，總商品金額";
$_LANG['consulting_service'] = "諮詢客服";
$_LANG['characteristic_theme'] = "特色主題";
$_LANG['modules_txt_promo'] = "促銷活動";
$_LANG['all_b2b_goods'] = "全部批發商品";
$_LANG['b2b_home'] = "批發首頁";
$_LANG['want_buy_info'] = "求購信息";
$_LANG['settlement_page'] = "結算頁";
$_LANG['fillin_order_info'] = "填寫訂單信息";
$_LANG['submit_order_success'] = "成功提交訂單";
$_LANG['selection_brand'] = "精選大牌";
$_LANG['brand_flagship'] = "品牌旗艦";

$_LANG['gift_gard_notic'] = "每個禮品卡僅支持提貨一次，提貨成功後該禮品卡即失效，禮品卡商品不支持退換貨，請謹慎選擇";
$_LANG['Apply_brand_two'] = "不存在，請點擊「提交品牌資質」";
$_LANG['not_voucher'] = "暫無可領取券";
$_LANG['no_record'] = "暫無記錄";
$_LANG['snatch_price_null'] = "此活動還沒有出價記錄喲~";
$_LANG['notic_store'] = "很抱歉，該區域暫無門店有貨，正努力補貨中•••";
$_LANG['talk_content_notic'] = "該對話被管理員屏蔽";

//goods lbi
$_LANG['shanghai'] = "上海";
$_LANG['beijing'] = "北京";
$_LANG['chongqing'] = "重慶";
$_LANG['nanjing'] = "南京";

$_LANG['register_button'] = '我要開店';
$_LANG['user_center'] = '進入會員中心';

$_LANG['vip_entrance'] = '快捷入口';
$_LANG['my_history_list'] = '我的瀏覽';
$_LANG['my_collection_list'] = '我的收藏';
$_LANG['my_order_list'] = '我的訂單';
$_LANG['my_account_safe'] = '賬號安全';
$_LANG['my_affiliate'] = '我要分享';
$_LANG['my_merchants'] = '商家入駐';

$_LANG['settled_step'] = '入駐流程';
$_LANG['settled_agreement'] = '入駐協議';
$_LANG['settled_help'] = '入駐幫助';
$_LANG['understand_more'] = '了解更多';
$_LANG['more_cat'] = '更多分類';

$_LANG['specified_time'] = '指定時間';
$_LANG['maximum_speed'] = '極速達';
$_LANG['begin_minute'] = '即將開始';
$_LANG['immediate_rush_buy'] = '立即搶購';

$_LANG['city_alt'] = '城市';
$_LANG['region'] = '區域';

$_LANG['hot_sale_list'] = '熱銷榜';
$_LANG['promotion_list'] = '促銷榜';
$_LANG['today_recommends'] = '今日推薦';
$_LANG['today_deal'] = '今日特價';
$_LANG['new_product'] = '新品到貨';
$_LANG['every_day'] = '新品到貨';
$_LANG['purchase_hand'] = '隨手購';
$_LANG['free_freight'] = '免運費';
$_LANG['express'] = '快遞';
$_LANG['activity_center'] = '活動中心';
$_LANG['account_center'] = '賬戶中心';
$_LANG['confirm_shipping'] = '確認收貨';

$_LANG['setup_complete'] = '設為完成';
$_LANG['service_charge'] = '手續費';

$_LANG['go_pay'] = "去付款";
$_LANG['not_paid'] = "未支付";
$_LANG['Already_paid'] = "已支付";

$_LANG['contact_username'] = '聯繫人';
$_LANG['contact_phone'] = '聯繫電話';
$_LANG['address_list'] = '收貨地址';
$_LANG['is_invoice'] = "是否開發票";
$_LANG['inv_payee'] = '發票抬頭';
$_LANG['label_tax_id'] = '納稅人識別號';
$_LANG['purchase_handle'] = '其他備註';

$_LANG['my_import_bill'] = "我的進貨單";
$_LANG['view_import_bill'] = "查看進貨單";
$_LANG['jian_available_sale'] = "件可售";
$_LANG['no_supporter'] = "暫無支持者";
$_LANG['no_zc_goods'] = "暫無眾籌商品";

$_LANG['wholesale_order_notic'] = '主人，您現在還沒有採購訂單喲！';
$_LANG['refund_replacement_applied '] = '已申請退換貨';
$_LANG['refund_replacement_appliy'] = '申請退換貨';

/* lbi文件end */
$_LANG['With_a_gift'] = "滿贈";
$_LANG['Full_reduction'] = "滿減";
$_LANG['discount'] = "折扣";

$_LANG['self_motion_goods'] = "自動確認收貨";
$_LANG['auto_system'] = "系統";

$_LANG['and_more'] = '及以上';
$_LANG['use_limit'] = '可使用';

$_LANG['through'] = '通過';
$_LANG['surplus_type_0'] = '充值';
$_LANG['surplus_type_1'] = '提現';

//跨境多商戶
$_LANG['cat_rate'] = '分類稅率';
$_LANG['is_duty_free'] = '是否免稅';
$_LANG['is_duty_free_notice'] = '勾選是則該商品不計算綜合稅';
$_LANG['from_cross_border'] = "跨境貨源";
$_LANG['domestic_warehouse'] = "國內倉庫";
$_LANG['free_trade_zone'] = "自貿區";
$_LANG['shipping_from_abroad'] = "海外直郵";
$_LANG['select_country'] = "請選擇國家";
$_LANG['please_select'] = "--請選擇--";
$_LANG['country'] = "國家";
$_LANG['real_name'] = '真實姓名';
$_LANG['id_card'] = '身份證號';
$_LANG['input_name'] = '請輸入真實姓名';
$_LANG['input_user_id'] = '請輸入身份證號碼';
$_LANG['label_rate_price'] = '支付稅費：';
$_LANG['cfg_name']['limited_amount'] = "跨境購物車限制金額";
$_LANG['cfg_name']['duty_free'] = "免稅金額";
$_LANG['general_tax'] = "綜合稅費";
$_LANG['tax_fee'] = "稅費";

$_LANG['topic'] = "專題";
$_LANG['shopping_activity'] = "活動列表";
$_LANG['download_file'] = "下載批量CSV文件（%s）";
$_LANG['charset']['utf8'] = "國際化編碼（utf8）";
$_LANG['charset']['zh_cn'] = "簡體中文";
$_LANG['charset']['zh_tw'] = "繁體中文";
$_LANG['charset']['en_us'] = "美國英語";
$_LANG['charset']['en_uk'] = "英文";

$_LANG['jigong']['apply_for_failed'] = "申請售後失敗，請返回重試或聯繫管理員";
$_LANG['jigong']['handle_failed'] = "操作失敗，請返回重試或聯繫管理員";

$_LANG['user_money_select'] = "詳情請點擊進入會員中心-資金管理頁面查詢";

$_LANG['friendly']['opportunity'][0] = '剛剛';
$_LANG['friendly']['opportunity'][1] = '分鐘前';
$_LANG['friendly']['opportunity'][2] = '小時前';
$_LANG['friendly']['opportunity'][3] = '天前';
$_LANG['friendly']['opportunity'][4] = '月前';
$_LANG['friendly']['opportunity'][5] = '年前';
$_LANG['friendly']['opportunity'][6] = '時間不合法';
$_LANG['friendly']['opportunity'][7] = '秒前';
$_LANG['friendly']['opportunity'][8] = '今天';
$_LANG['friendly']['opportunity'][9] = '周前';
$_LANG['friendly']['opportunity'][10] = '個月前';

$_LANG['warehouse_goods_exits'] = '該商品的倉庫庫存已存在';
$_LANG['area_goods_exits'] = '該商品的地區價格已存在';

$_LANG['main_order_pay'] = "操作主訂單支付";
$_LANG['data_empty'] = '數據為空';
$_LANG['manually_create'] = '手動創建';

$_LANG['follow_wechat'] = '關注微信';
$_LANG['down_app'] = '下載APP';

return $_LANG;
