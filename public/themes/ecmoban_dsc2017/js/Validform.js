$.Validform = function(a,b,c){
	...
	nameValid = function(val,error){
		if(val == ''){
			...
			Ajax.call('ajax_user.php?act=is_user', 'username=' + val, function(data){
	...
	phoneValid = function(val,error){
		if(val == ''){
			...
			if(val != username_val){
				Ajax.call('ajax_user.php?act=check_phone', 'mobile_phone=' + val, function(result){
					if ( result == 'true' ){
						error.parent().prev().removeClass("form-item-error").addClass("form-item-valid");
						error.html('');
	...
	codeValid = function(val,error){
		if(val == ''){
			...
			Ajax.call('ajax_user.php?act=is_register_captcha', 'captcha=' + val, function(res){
				if (res.result == "false"){
					//验证验证码是否正确
					error.parent().prev().addClass("form-item-error").removeClass("form-item-valid");
					error.removeClass().addClass("error");
	...
	telcodeValid = function(val,error){
		if(val == ''){
			...
			Ajax.call('ajax_user.php?act=is_mobile_code', 'mobile_code=' + val, function(data){
				if (data.result == "true"){
					error.parent().prev().removeClass("form-item-error").addClass("form-item-valid");
					error.html('');
					return_mobile_code = 0;
