...
function change_SizePriceResponse(res) {
    if (res.err_msg) {
        ...
        if (res.product_number == 0) {
            size_goods.innerHTML = "<b>" + day_not_available + "</b>";
        } else {
            document.getElementById(res.pid).value = res.product_number;
            document.getElementById(res.pid).focus();
            changePrice(); //新增 2014-03-20
            size_goods.innerHTML = "<b>" + day_yes_available + "，<font style='font-size:14px; color:#003D79'>" + res.attr_value + res.size_name + "</font>" + inventory + "<font style='font-size:14px; color:#E6393D'>" + res.product_number + "</font>" + letter + "</b>";
        }
