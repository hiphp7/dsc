    ...
    $(document).ready(function (e) {
        ...
        function db_column() {
            ...
            $(document).on("click", "*[ectype='delete_bg']", function () {
                ...
                form.find(".bgimg img").attr("src", "__TPL__/data/gallery_album/visualDefault/bgimg.gif");
            ...
            $(document).on('click', '*[ectype="delete_adv"]', function () {
                ...
                Ajax.call('visualhome.php', "act=delete_adv&suffix=" + suffix, function (data) {
                    _this.parents('.page-head-bgimg').find('img').attr('src', "__TPL__/data/gallery_album/visualDefault/bgimg.gif");
        ...
        $(".module-list .lyrow").draggable({
            connectToSortable: ".demo", helper: "clone", handle: ".drag",
            drag: function (e, t) {
                t.helper.width(63);
            },
            stop: function (e, t) {
            ...
            if (confirm("确定删除此广告么？删除后前台不显示只能后台编辑，且不可找回！")) {
                Ajax.call('visualhome.php', "act=model_delete&suffix=" + suffix, function (data) {
                    if (data.error == 1) {
                        ...
                        obj.find('[data-type="range"] img').attr("src", "__TPL__/data/gallery_album/visualDefault/homeIndex_011.jpg");
    ...
    visualShell.on("click", "*[ectype='model_edit']", function () {
        ...
        switch (purebox) {
            ...
            case "goods":
                ...
                //商品模块编辑
                spec_attr = JSON.stringify(spec_attr);
                Ajax.call('dialog.php', "act=goods_info" + "&mode=" + mode + "&diff=" + diff + "&spec_attr=" + encodeURIComponent(spec_attr) + "&lift=" + lift + "&suffix=" + suffix + "&current_url=" + url, query_goods, 'POST', 'JSON');
                break;
...
//            spec_attr.custom_content = custom_content;
        ...
        query_banner = function (result) {
            ...
            function addshop_banner(obj, mode, diff) {
                ...
                spec_attr = $.toJSON(spec_attr).replace(/&/g, "%26"); //处理&转义
                Ajax.call('get_ajax_content.php', "act=addmodule&diff=" + diff + "&mode=" + mode + "&spec_attr=" + spec_attr, addmoduleResponse, 'POST', 'JSON');
            }
    ...
    function dialogFloorCate() {
        ...
        $(document).on("click", setupGoods, function () {
            ...
            Ajax.call('dialog.php', "act=goods_info" + "&goods_type=1&cat_id=" + cat_id + "&spec_attr=" + $.toJSON(spec_attr) + "&good_number=" + good_number + "&current_url=" + url, function (result) {
    ...
    function f_defaultBrand() {
        ...
        var html = '<div class="item"><a href="#" target="_blank"><div class="link-l"></div><div class="img"><img src="__TPL__/data/gallery_album/visualDefault/homeIndex_010.jpg" title="esprit"></div><div class="link"></div></a></div>';
        var i = 0;
        for (i = 0; i < 9; i++) {
            _this.append(html);
        }
        ...
        if (suffix) {
            ...
            if (section == "vis_home") {
                if (temp == 1) {
                    ...
                    preview.find("*[ectype='user_info']").html("<i class='icon-spin'><img src='/storage/data/gallery_album/visualDefault/load_spin.gif' width='30' height='30'></i>");
                    ...
                    if (suffix == "backup_festival_1") {
                        ...
                        preview.find(".box-bd").html("<i class='icon-spin'><img src='/storage/data/gallery_album/visualDefault/load_spin.gif' width='30' height='30'></i>");
                    }
