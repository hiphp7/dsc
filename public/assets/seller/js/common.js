...
/* 检查新订单的时间间隔 */
var NEW_ORDER_INTERVAL = 300000; // 每5分钟
...
function set_attribute_table(goodsId , type, city_id)
{
    ...
    var area_id = $("#attribute_model").find("input[type=radio][data-type=region_id]:checked").val();
    var city_id = $("#attribute_model").find("input[type=radio][data-type=city_region_id]:checked").val();
	...
	if(goods_model == 1){
		...
		if($("#attribute_city_region").length > 0){
			extension += "&warehouse_id="+warehouse_id + "&region_id="+area_id + "&city_id="+ city_id;
		}else{
			extension += "&warehouse_id="+warehouse_id + "&region_id="+area_id;
		}
	}
