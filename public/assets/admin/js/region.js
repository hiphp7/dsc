...
jQuery.levelLink = function () {
    ...
    var domain = fullPath.substring(0, fullPath.substr(1).indexOf('/') + 1);
    ...
    if ($('.ui-dropdown').parents(".level_linkage").length > 0) {
        $(".level_linkage").each(function () {
            ...
            $this.find('.ui-dropdown').each(function (v, k) {
                ...
                $.jqueryAjax(domain + 'region.php', 'type=' + (v + 1) + '&parent=' + val, function (data) {
    ...
    $(document).on('click', liv, function () {
        ...
        $.jqueryAjax(domain + 'region.php', 'type=' + type + '&parent=' + value, function (data) {
            if (data.content) {
