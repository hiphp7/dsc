...
listTable.edit = function(obj, act, id)
{
  ...
  txt.onblur = function(e)
  {
    ...
    {
      res = Ajax.call(listTable.url, "act="+act+"&val=" + encodeURIComponent(Utils.trim(txt.value)) + "&id=" +id, null, "POST", "JSON");
 ...
 * 删除列表中的一个记录
 */
listTable.remove = function(id, opt)
{
  if (opt == null)
  {
    opt = "remove";
 ...
 * input编辑 by wu
 */
listTable.editInput = function(obj, act, id)
{
	var value = obj.value
    if (Utils.trim(value).length > 0)
    {
      res = Ajax.call(listTable.url, "act="+act+"&val=" + encodeURIComponent(Utils.trim(value)) + "&id=" +id, null, "POST", "JSON", false);
