...
var dscmallKefu = {
    ...
    connect: function () {
        ...
        this.socket.onopen = function () {
            that.socket.send(that.json_decode({
                uid: that.user.user_id,
                name: that.user.user_name,
                type: 'login',
                user_type: that.user.user_type,
                avatar: that.user.avatar,
                store_id: that.user.store_id,
                origin: (that.IsPC()) ? 'PC' : 'H5'
            }));

            // 客户端定时发送心跳 15 秒
            clearInterval(timer);
            timer = setInterval(function () {
                that.socket.send('{"type":"ping"}');
            }, 15 * 1000);
        };
    ...
    sendmsg: function () {
        ...
        dscmallEvent.add_message({
            ...
            time: dscmallKefu.SystemDate(),
            origin: dscmallKefu.come_form
        }, 1);
    },
    //
