...
var dscmallEvent = {
    ...
    service_list_chat_data: function (service_id) {
        ...
        $.ajax({
            ...
            success: function (res) {
                ...
                // for ( let i in res.chat ) {
                // res.chat.push(res.chat[i]);
                // }
                // }
    ...
    add_message: function (data, warp_chat) {
        ...
        if(data.user_type == 'customer'){
            var service_id = data.to_id || this.vueobj.current_target;
        }else {
            var service_id = data.from_id || this.vueobj.current_target;
        }
        ...
        this.vueobj = new Vue({
            ...
            methods: {
                change_service: function (service_id, event) {
                    ...
                    var store_id = (event == undefined) ? dscmallKefu.user.store_id : event.currentTarget.getAttribute("data-ruid");
            ...
            computed: {
                evenNumbers: function () {
                    if (this.service_list_chat_data[this.current_target] == undefined) return;
            ...
            filters: {
                filter_shop_name: function (value) {
                    ...
                    return value.indexOf("自营") === -1 ? "店铺" : "自营";
                },
    ...
    get_service: function (data) {
        ...
        for (let i in this.vueobj.service_list) {
            if (this.vueobj.service_list[i].ru_id == data.store_id &&
                (this.vueobj.service_list[i].service_id === 0)
            ) {
    ...
    get_more_msg: function (page, store_id) {
        ...
        $.ajax({
            ...
            success: function (res) {
                let data = res; // dscmallKefu.json_encode(res)
