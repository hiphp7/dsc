...
class WeappModuleSeeder extends Seeder
{
	...
	private function adminAction()
    {
		...
		$result_1 = DB::table('admin_action')->where('action_code', 'wxapp_config')->count();
		if (!empty($result_1)) {
			// 删除旧数据
    ...
    {
        $result = DB::table('wxapp_template')->where('wx_code', 'AT0541')->count();
		$result_1 = DB::table('wxapp_template')->where('wx_code', 'AT1173')->count();
		$result_2 = DB::table('wxapp_template')->where('wx_code', 'AT0933')->count();
