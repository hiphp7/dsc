...
class CreateWholesaleOrderReturnTable extends Migration
{
    ...
    public function up()
    {
        $name = 'wholesale_order_return';
        if (Schema::hasTable($name)) {
            return false;
        }
        Schema::create($name, function (Blueprint $table) {
            $table->increments('ret_id')->comment('退换货id');
            $table->string('return_sn', 20)->comment('退换货单号');
            ...
            $table->boolean('credentials')->default(0)->comment('申请凭据');
            $table->boolean('maintain')->default(0)->comment('维修标识');
            $table->boolean('back')->default(0)->comment('退货标识');
            $table->text('goods_attr', 65535)->comment('退货颜色属性');
            $table->boolean('exchange')->default(0)->comment('换货标识');
            $table->boolean('return_type')->default(0)->comment('退换货标识：0：维修，1：退货，2：换货，3：仅退款');
            $table->text('attr_val', 65535)->comment('换货属性');
            $table->integer('cause_id')->comment('退换货原因');
            $table->string('apply_time', 120)->comment('退换货申请时间');
            $table->string('return_time', 120)->comment('退换货时间');
            $table->decimal('should_return', 10)->unsigned()->default(0.00)->comment('应退金额');
            $table->decimal('actual_return', 10)->unsigned()->default(0.00)->comment('实退金额');
            $table->decimal('return_shipping_fee', 10)->unsigned()->comment('退换货配送金额');
            $table->string('return_brief', 2000)->comment('退换货描述');
            $table->string('remark', 2000)->comment('备注');
            $table->smallInteger('country')->comment('国家');
            $table->smallInteger('province')->comment('省份');
            $table->smallInteger('city')->comment('城市');
            $table->smallInteger('district')->comment('区');
            $table->smallInteger('street')->unsigned()->default(0)->comment('街道');
            ...
            $table->boolean('refund_type')->default(1)->comment('判断退换货订单退款方式');
        });

        $prefix = config('database.connections.mysql.prefix');
        DB::statement("ALTER TABLE `" . $prefix . "$name` comment '供应链退换货订单'");
    }
    ...
    {
        Schema::drop('wholesale_order_return');
    }
